...
def test_resolver_returns_fastener_category_for_gb_bolt() -> None:
    ...
    first-hit-wins 下命中 match.category=fastener 的某条规则
    （可能是 sw_toolbox GB 头，可能 fall-through 到 bd_warehouse 内六角头；
    无论哪条，rule["match"]["category"] == "fastener" 都推断出 STANDARD_FASTENER）。
    """
    r = default_resolver(project_root=".")
    q = _query(
        name_cn="GB/T 70.1 M6×20 内六角",
        category="fastener",
        material="不锈钢",
    )
    res = r.resolve(q)
