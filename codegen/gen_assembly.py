...
def _load_product_graph_instance_map(spec_path: str) -> dict[str, list[str]]:
    ...
    if not isinstance(instances, list):
        raise ValueError(f"Invalid PRODUCT_GRAPH.json: instances must be a list: {graph_path}")
    seen_instance_ids = set()
    for instance in instances:
        if not isinstance(instance, dict):
            raise ValueError(f"Invalid PRODUCT_GRAPH.json: instance must be an object: {graph_path}")
        if instance.get("render_policy") == "excluded":
            continue
        part_no = instance.get("part_no")
        instance_id = instance.get("instance_id")
        if part_no and instance_id:
            if instance_id in seen_instance_ids:
                raise ValueError(
                    f"Invalid PRODUCT_GRAPH.json: duplicate instance_id {instance_id}: {graph_path}"
                )
...
def _placement_rows_for_part(
    part_no: str,
    part_positions: dict,
    product_graph_instances: dict[str, list[str]],
) -> list[dict] | list[None]:
    positioned_rows = list(part_positions.get(part_no, {}).get("instances", []))
    contract_ids = product_graph_instances.get(part_no, [])
    if contract_ids:
        rows = []
        for idx, instance_id in enumerate(contract_ids):
            row = dict(positioned_rows[idx]) if idx < len(positioned_rows) else {}
            row["instance_id"] = instance_id
            row["_contract_instance_id"] = True
            rows.append(row)
        return rows
    return positioned_rows or [None]
...
def _assembly_name(
    ...
    *,
    is_std: bool,
    contract_instance: bool,
) -> str:
    if contract_instance and inst_id:
        return inst_id
    if is_std:
        return f"STD-{part_no}{('-' + inst_id) if inst_id else ''}"
    return f"{legacy_base}{('-' + inst_id) if inst_id else ''}"
...
def generate_assembly(spec_path: str) -> str:
    ...
    product_graph_instances = _load_product_graph_instance_map(spec_path)
        ...
        for child in children:
            ...
            if "外购" in make_buy or "标准" in make_buy or library_routed:
                ...
                for inst in placement_rows:
                    ...
                    contract_instance = bool(inst and inst.get("_contract_instance_id"))
