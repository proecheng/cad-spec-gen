...
```bash
# Image-to-image enhancement — unified template, view-type auto-switches
python gemini_gen.py --prompt-file prompt.txt --image V1_front_iso.png
# prompt.txt is generated by: python tools/hybrid_render/prompt_builder.py --config cad/end_effector/render_config.json --view V1
```
...
3. Select prompt template per view type:
   - All views → `templates/prompt_enhance_unified.txt` (unified template)
   - View-specific content auto-switches via `render_config.json` camera.type field
   - Assembly/material data auto-generated from `params.py` by `prompt_data_builder.py`
