...
the spec data. Writes ASSEMBLY_REPORT.json and, when PRODUCT_GRAPH.json
is present, ASSEMBLY_SIGNATURE.json.
...
def extract_part_transforms(assy) -> dict:
    """Extract best-effort per-part transforms for ASSEMBLY_SIGNATURE.json."""
    transforms = {}
    for name, sub in assy.objects.items():
        transforms[name] = {
            "translation_mm": _translation_from_location(getattr(sub, "loc", None)),
...
def _translation_from_location(loc) -> list[float]:
    if loc is None:
        return [0.0, 0.0, 0.0]
    try:
        data = loc.toTuple()
        if isinstance(data, tuple) and data and len(data[0]) == 3:
            return [float(v) for v in data[0]]
    except Exception:
        pass
    try:
        vector = loc.wrapped.Transformation().TranslationPart()
...
def validate_assembly(sub_dir: str, spec_path: str = None,
    ...
    try:
        ...
        make_assembly = load_make_assembly(sub_dir)
    except ImportError as e:
        return {"error": f"Cannot import assembly loader: {e}"}
    ...
    transforms = extract_part_transforms(assy)
    ...
    project_root = Path(sub_dir).resolve().parents[1]
    try:
        product_graph_path, product_graph = _load_product_graph_for_validation(sub_dir)
        out_dir = _run_output_dir(sub_dir, output_dir, product_graph, project_root)
...
def _load_product_graph_for_validation(sub_dir: str) -> tuple[str, dict | None]:
    product_graph_path = os.path.join(sub_dir, "PRODUCT_GRAPH.json")
    if not os.path.isfile(product_graph_path):
        return product_graph_path, None
    try:
        with open(product_graph_path, encoding="utf-8") as f:
            product_graph = json.load(f)
    except json.JSONDecodeError as exc:
        raise ValueError(f"Invalid PRODUCT_GRAPH.json: {product_graph_path}: {exc}") from exc
    if not isinstance(product_graph, dict):
        raise ValueError(f"Invalid PRODUCT_GRAPH.json: expected object: {product_graph_path}")
    return product_graph_path, product_graph


def _run_output_dir(
    sub_dir: str,
    output_dir: str | None,
    product_graph: dict | None,
    project_root: Path,
) -> str:
    base = Path(output_dir) if output_dir else Path(sub_dir) / ".." / "output"
    base = base.resolve()
    try:
        base.relative_to(project_root)
    except ValueError as exc:
        raise ValueError(f"output_dir must be within project: {base}") from exc
    run_id = _safe_run_id((product_graph or {}).get("run_id"))
    run_dir = (base / "runs" / run_id).resolve()
    try:
        run_dir.relative_to(project_root)
    except ValueError as exc:
        raise ValueError(f"run_id must resolve within project output: {run_id}") from exc
    return str(run_dir)


def _safe_run_id(raw_run_id: object) -> str:
    if raw_run_id in (None, ""):
        return datetime.now().strftime("run_%Y%m%d_%H%M%S")
    run_id = str(raw_run_id)
    if Path(run_id).is_absolute() or "/" in run_id or "\\" in run_id or run_id in {".", ".."}:
        raise ValueError(f"Invalid run_id for output path: {run_id}")
...
def main():
    ...
    print(f"  Report: {report.get('report_path', 'ASSEMBLY_REPORT.json')}")
