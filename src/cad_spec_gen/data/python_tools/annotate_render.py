...
def main():
    ...
    if args.manifest:
        ...
        _render_dir = _mdata.get("render_dir") or _mdata.get("render_dir_abs_resolved") or "."
        _raw_pngs = []
        for _entry in _mdata.get("files", []):
            if isinstance(_entry, str):
                _raw_pngs.append(_entry)
            elif isinstance(_entry, dict):
                _path = (
                    _entry.get("path_abs_resolved")
                    or _entry.get("path")
                    or _entry.get("path_rel_project")
                )
                if _path:
                    _raw_pngs.append(_path)
        if not _raw_pngs:
            _raw_pngs = list(_mdata.get("legacy_files", []))
