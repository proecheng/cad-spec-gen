...
""",
    ...
    p_enhance_check = sub.add_parser(
        ...
        description=(
            ...
            "enhanced image in the same directory, computes deterministic "
            "multi-view quality metrics, and writes ENHANCEMENT_REPORT.json "
            "with quality_summary."
        ),
        epilog=(
            ...
            "enhanced image, consistency passes, and quality_summary is accepted; "
            "preview = enhanced images exist but shape/QA or multi-view quality "
            "needs review; blocked = required views or "
            ...
            "generic mask-IoU threshold. quality_summary records deterministic "
            "multi-view quality evidence such as canvas consistency, contrast, "
            "luminance, saturation, and occupancy; it is not semantic AI judgment."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    ...
    p_project_guide = sub.add_parser(
        ...
        description=(
            ...
            "photo3d-handoff --provider-preset 预览命令；如果存在 "
            "geometry_report.json，还会嵌入 model_quality_summary，展示每个 "
            "model source / A-E 质量等级 / 真 STEP、通用模板或 fallback 的 "
            "普通用户可读状态。"
        ...
        epilog=(
            ...
            "if cad/<subsystem>/.cad-spec-gen/geometry_report.json exists, it also "
            "embeds model_quality_summary with ordinary-user model source labels, "
            "readiness_status, photoreal_risk, and per-part review actions. "
    ...
    p_photo3d = sub.add_parser(
        ...
        epilog=(
            ...
            "enhancement consistency plus quality_summary pass; preview = CAD gate "
            "pass but enhancement consistency or multi-view quality is unverified "
            "or failed; blocked = CAD gate failed. After "
    ...
    p_photo3d_deliver = sub.add_parser(
        ...
        description=(
            ...
            "ENHANCEMENT_REVIEW_REPORT.json（如存在）、active-run MODEL_CONTRACT.json "
            ...
            "不作为照片级最终交付。交付包同时写 model_quality_summary，说明 "
            "active-run model quality、每个零件的模型来源、质量等级和复核建议。"
        ),
        epilog=(
            ...
            "beside DELIVERY_PACKAGE.json. When active-run MODEL_CONTRACT.json is "
            "present, DELIVERY_PACKAGE.json includes model_quality_summary derived "
            "from that same-run contract, not from a stale project-level report. "
            "Use --include-preview only when an "
