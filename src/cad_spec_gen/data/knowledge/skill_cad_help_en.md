...
Determine user needs then execute or guide:

```
Case A: User wants to render directly → Run commands
  # Blender Cycles rendering (geometrically accurate PNG)
  cd cad/<subsystem> && python build_all.py --render

  # Or render a specific view individually
...
Case B: User does not have GLB yet → Guide them to build first
  1. python cad/end_effector/build_all.py  → Generate STEP + DXF
  2. assembly.py in build_all.py exports GLB
  3. Then --render triggers Blender

Case C: User wants to render another subsystem → Check if render_config.json exists
```
