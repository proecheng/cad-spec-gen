...
Recommended first command for new ordinary users and LLM agents:
...
`project-guide --from-design-doc` is the design-document-only entry mode. It reads exactly one explicit design document, writes `.cad-spec-gen/project-guide/PROJECT_GUIDE.json`, returns `needs_subsystem_confirmation`, and sets `next_action.kind` to `confirm_subsystem`. It only suggests subsystem candidates and preview `project-guide --subsystem ... --design-doc ...` commands; user confirmation is required before any subsystem-scoped guide runs. It does not scan `docs/design`, does not scan `cad/`, does not guess the newest run, does not initialize a subsystem, and does not generate spec/code/render/enhance artifacts.

After the subsystem is confirmed, `project-guide` is a read-only project guide. It writes `PROJECT_GUIDE.json`, checks the explicit subsystem, optional design document, fixed `CAD_SPEC.md` / codegen sentinels, and the explicitly resolved `ARTIFACT_INDEX.json` active run. It selects the next safe handoff across `init`, `spec`, `codegen`, `build --render`, and `photo3d-run`. If `cad/<subsystem>/.cad-spec-gen/geometry_report.json` exists, `PROJECT_GUIDE.json` also embeds `model_quality_summary`, a user-facing model source summary with A-E quality, real STEP / library template / fallback labels, `readiness_status`, `photoreal_risk`, per-part review actions, and no directory scanning. When the current active run is already `ready_for_enhancement`, `PROJECT_GUIDE.json` may include read-only provider preset choices, display-ready `ordinary_user_options`, `provider_wizard`, `provider_health`, and preview `photo3d-handoff --provider-preset <id>` commands for ordinary users or LLM agents. `ordinary_user_options` contains titles, summaries, recommended-use text, setup requirements, safe provider health status, and preview argv so users choose "default / local engineering preview / cloud enhancement" instead of writing backend flags. `provider_wizard` organizes those allowlisted options into display steps, default selection, preview actions, and health summary for UI or LLM clients. `provider_health` checks local config/dependency presence only; it must not execute enhancement, mutate state, scan output directories, or expose API key values, environment variable names, URLs, endpoints, secrets, arbitrary backend strings, model names, or JSON argv injection. It does not scan directories, does not mutate pipeline state, does not accept baseline, does not append `--confirm`, and does not run enhancement.
...
`photo3d-handoff` reads the current active run's `PHOTO3D_RUN.json` or `PHOTO3D_AUTOPILOT.json` and writes `PHOTO3D_HANDOFF.json`. Preview mode is the default. With `--confirm`, it executes only recognized current next actions: `accept-baseline`, `enhance`, `enhance-check`, or `photo3d-run --confirm-actions`. It does not scan directories, never trusts arbitrary argv from JSON reports, and rebuilds argv from `ARTIFACT_INDEX.json`, the current `active_run_id`, and the current run/render paths. For enhancement, `--provider-preset default|engineering|gemini|fal|fal_comfy|comfyui` is an allowlisted provider preset mapped only to supported `enhance --backend/--model` flags; do not pass arbitrary backend strings, URLs, API keys, or future model names through JSON argv. After confirmed `enhance` succeeds, handoff automatically runs `enhance-check` for the same active run, records the subprocess result in `followup_action`, then reruns `photo3d-run` once without confirmation and stores the resulting accepted/preview/blocked delivery state in `post_handoff_photo3d_run`. `executed_with_followup` means the enhancement command and same-run acceptance follow-up both completed; if `enhance-check` returns nonzero but writes a valid blocked `ENHANCEMENT_REPORT.json`, that blocked state is surfaced through `PHOTO3D_RUN.json` rather than hidden as a raw subprocess failure. Delivery-complete, preview-review, user-input, manual-review, and unknown actions remain manual and are recorded in `PHOTO3D_HANDOFF.json`.
...
`photo3d-deliver` reads only the current `ARTIFACT_INDEX.json.active_run_id` and the same-run `render_manifest.json`, `ENHANCEMENT_REPORT.json`, optional `ENHANCEMENT_REVIEW_REPORT.json`, `PHOTO3D_RUN.json`, active-run `MODEL_CONTRACT.json`, and contract evidence. It writes `cad/<subsystem>/.cad-spec-gen/runs/<run_id>/delivery/DELIVERY_PACKAGE.json` plus `README.md`. The package includes `model_quality_summary` derived from that same-run `MODEL_CONTRACT.json`, so ordinary users can see active-run model source labels, A-E quality, fallback risk, and part-level review actions instead of trusting a stale project-level report. By default it copies final enhanced images, source renders, and unambiguous labeled images only when delivery status is `accepted` and `quality_summary.status` is `accepted`; `preview` / `blocked` or unaccepted quality produce an evidence report but are not marked `final_deliverable`. When quality is not accepted, it records `photo_quality_not_accepted`. If same-run `semantic_material_review` exists and is not `accepted`, final delivery is blocked with `semantic_review_not_accepted`. Use `--require-semantic-review` when final delivery must require accepted `semantic_material_review`; missing review then records `semantic_review_required`. It does not scan directories for newest files, infer another run, or accept subsystem/run_id/render_manifest drift. Use `--include-preview` only for an explicit preview package; `final_deliverable` remains false.

Outputs for ordinary users and LLMs:

- `PROJECT_GUIDE.json`: read-only project-level next-step report. Entry mode uses `--from-design-doc` to write `.cad-spec-gen/project-guide/PROJECT_GUIDE.json` with `needs_subsystem_confirmation` and `confirm_subsystem`; after subsystem confirmation it reports init/spec/codegen/build-render/photo3d-run handoffs. When `geometry_report.json` exists, it includes `model_quality_summary` with model source labels, A-E quality, readiness status, photoreal risk, and per-part review actions; when the active run is ready for enhancement, it may include allowlisted provider preset choices, display-ready `ordinary_user_options`, `provider_wizard`, safe `provider_health`, and preview `photo3d-handoff --provider-preset <id>` commands.
...
- `DELIVERY_PACKAGE.json`: final delivery manifest from `photo3d-deliver`, including source reports, source renders, enhanced images, labeled images, copied evidence files, quality summary, active-run `model_quality_summary` from `MODEL_CONTRACT.json`, optional `semantic_material_review`, blocking reasons such as `photo_quality_not_accepted`, and `final_deliverable` status.
