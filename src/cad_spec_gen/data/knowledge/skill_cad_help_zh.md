...
`project-guide` is a read-only project guide. It writes `PROJECT_GUIDE.json`, checks the explicit subsystem, optional design document, fixed `CAD_SPEC.md` / codegen sentinels, and the explicitly resolved `ARTIFACT_INDEX.json` active run. It selects the next safe handoff across `init`, `spec`, `codegen`, `build --render`, and `photo3d-run`. When the current active run is already `ready_for_enhancement`, `PROJECT_GUIDE.json` may include read-only provider preset choices, display-ready `ordinary_user_options`, `provider_wizard`, and preview `photo3d-handoff --provider-preset <id>` commands for ordinary users or LLM agents. `ordinary_user_options` contains titles, summaries, recommended-use text, setup requirements, and preview argv so users choose "default / local engineering preview / cloud enhancement" instead of writing backend flags. `provider_wizard` organizes those allowlisted options into display steps, default selection, and preview actions for UI or LLM clients. Those choices come only from the allowlist and must not accept arbitrary backend strings, URLs, API keys, model names, or JSON argv injection. It does not scan directories, does not mutate pipeline state, does not accept baseline, does not append `--confirm`, and does not run enhancement.
...
- `PROJECT_GUIDE.json`: read-only project-level next-step report across init/spec/codegen/build-render/photo3d-run; when the active run is ready for enhancement, it may include allowlisted provider preset choices, display-ready `ordinary_user_options`, `provider_wizard`, and preview `photo3d-handoff --provider-preset <id>` commands.
