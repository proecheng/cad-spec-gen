...
- **Parts Library System** (v2.8.0+): Purchased part geometry source is driven by a `parts_library.yaml` registry — supports three backends: local STEP pool (`std_parts/`) / `bd_warehouse` parametric / `partcad` package manager, plus the `jinja_primitive` terminal fallback. Phase 1 P7 envelope backfill writes library-probed sizes into §6.4; Phase 2 codegen uses `resolver.resolve()` to decide each `make_std_*()` body form (codegen / step_import / python_import). Without a yaml the system is a no-op and output is byte-identical to v2.7.x. Kill switch: `CAD_PARTS_LIBRARY_DISABLE=1`
- **Registry Inheritance + Coverage Report** (v2.8.1+): A `parts_library.yaml` with `extends: default` inherits the skill-shipped default rules, with project mappings prepended. `gen_std_parts.py` prints a per-adapter coverage table at the end of code generation, showing which parts use a real library and which fell back to jinja, plus a hint on how to upgrade them
