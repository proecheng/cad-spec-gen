...
3. **Photo3D / 照片级一键出图请求**（当用户说 project-guide、photo3d、photo3d-run、photo3d-autopilot、photo3d-action、render-visual-check、render-quality-check、照片级、傻瓜式出图、pass/warning/blocked、accepted/preview/blocked、run_id、baseline、动作计划或让其他大模型继续时）：
   ...
   - 大模型优先运行/读取 `photo3d-run` 和 `PHOTO3D_RUN.json`；Phase 4 证据要分清：`render-visual-check` 证明视角/元件契约，`render-quality-check` 证明 Blender 环境和截图像素质量，二者都只读当前 active run。用户说“按建议执行”时优先走 `photo3d-handoff` 预览/确认交接。用户要选增强后端时只用 `--provider-preset` 白名单值，例如 `engineering`，不要手写 `--backend` 或复制 JSON 里的任意 argv。增强确认执行后读取 `PHOTO3D_HANDOFF.json.post_handoff_photo3d_run` 和 `ENHANCEMENT_REPORT.json`，不要扫描 render 目录猜最新增强图；当状态为 `enhancement_accepted` 时运行 `photo3d-deliver` 生成 `DELIVERY_PACKAGE.json`，不要手工复制图片。需要分步执行 blocked 恢复动作时只能依据 `ACTION_PLAN.json` 执行。低风险 CLI 恢复动作走 `photo3d-action` 预览/确认执行，底层命令必须经 `photo3d-recover --run-id <run_id> --artifact-index <path>` 绑定当前 run。如果动作需要用户输入，询问用户，不要虚构路径或模型。
