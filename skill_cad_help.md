...
- `PROJECT_GUIDE.json`: read-only project-level next-step report across init/spec/codegen/build-render/photo3d-run; when `geometry_report.json` exists, it includes `model_quality_summary` with model source labels, A-E quality, readiness status, photoreal risk, and per-part review actions; when the active run is ready for enhancement, it may include allowlisted provider preset choices, display-ready `ordinary_user_options`, `provider_wizard`, safe `provider_health`, and preview `photo3d-handoff --provider-preset <id>` commands.
