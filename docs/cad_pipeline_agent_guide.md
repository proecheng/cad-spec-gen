...
### 3.1a fasteners.py — 标准紧固件与弹簧库

`cad/<subsystem>/fasteners.py` 提供通用紧固件和弹簧几何生成函数：

| 函数 | 说明 |
|------|------|
| `make_hex_socket_bolt(m, length)` | ISO 4762 内六角螺栓（M2-M8） |
| `make_bolt_ring(m, length, pcd, n)` | N颗螺栓等分布于 PCD 圆 |
| `make_bolt_square(m, length, pcd)` | 4颗螺栓方形布局 |
| `make_lemo_0b(bore_dia)` | LEMO 0B 连接器简化模型 |
| `make_helix_spring(od, wire_dia, free_length, n_coils)` | **螺旋弹簧**（Wire.makeHelix + sweep） |
| `make_compression_spring(od, wire_dia, free_length, n_coils)` | 压缩弹簧（堆叠环体近似，作为 helix 的 fallback） |

`make_helix_spring()` 使用 CadQuery 的 `Wire.makeHelix()` 创建真实螺旋线，再用 `sweep(isFrenet=True)` 扫掠圆形截面。若当前 CadQuery 版本不支持，自动回退到 `make_compression_spring()`（堆叠环体）。
