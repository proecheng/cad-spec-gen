...
| --- | --- |
| 更新日期 | 2026-05-05 |
| 当前工作分支 | `main`：Phase 2 常用模型库第五批已快进合并并通过主线复验，待推送和 worktree/临时分支清理 |
| 管线 Phase 数 | 6 个：SPEC / CODEGEN / BUILD / RENDER / ENHANCE / ANNOTATE |
| 总体能力进展 | 约 84%（按 6 个 Phase 的工程化能力估算，不代表某个具体产品一次出图进度） |
| 当前主攻 Phase | Phase 2 CODEGEN：按通用模型族准入清单扩展第五批跨产品外购件 B 级模板 |
| 最新功能基线 | 常用模型库第五批：方形法兰伺服电机、行星减速机、拖链段、DIN 导轨继电器模块、按钮/操作盒，均要求显式类别和族意图 |
| 最新合并/进度提交 | `86948c3 feat(parts-library): 扩展常用模型库第五批` 已快进合并到 `main`；本看板收尾记录待提交 |
| 最新归档计划提交 | `9ed3280 docs(project): 归档通用传动件计划` |
| 最近验证 | 合并到 `main` 后最终范围门禁：`537 passed, 2 skipped, 11 warnings`；`git diff --check` 通过 |
| 同步检查 | 合并到 `main` 后先同步 3 个本地安装镜像，再运行 `python scripts\dev_sync.py --check` 通过；镜像仍按 `.gitignore` 生成区处理 |
| 当前未跟踪 | 无运行时产物纳入。另有独立旧 worktree `.worktrees/generic-threaded-photo-autopilot`，本轮不触碰 |

## 一句话结论

cad-spec-gen 已形成 6 阶段 CAD 混合渲染管线。现在不是围绕某个零件做临时调参，而是在把“其他用户、其他产品也能傻瓜式出照片级 3D 图”的通用流程补齐：本轮回到 Phase 2，继续扩展默认常用模型库，但所有新增族都走显式类别、显式族意图、准入清单和几何包络测试，不让宽泛词汇把无关零件误吸进模板。
...
| Phase 2 CODEGEN | 从 Spec 生成 CadQuery 代码、BOM 路由和标准件模型 | `params.py`、`build_all.py`、`assembly.py`、`std_*.py` | 84% | 模型库、resolver、五批常用模型族和准入清单已成型；仍需继续扩展跨产品高频件和模型质量可读报告 |
...
| Phase 6 ANNOTATE / DELIVER | 标注、交付最终图片包和证据摘要 | `*_labeled_*.png`、`DELIVERY_PACKAGE.json`、`semantic_material_review`、可读摘要 | 64% | `photo3d-deliver` 已生成 active-run 最终包，并可要求增强状态、`quality_summary` 和 `semantic_material_review` 同时 accepted；后续仍需最终报告美化和缩略图/可视化验收 |

**总体进展：约 84%。** 最大剩余缺口不在单个零件，而在 Phase 2 的跨产品模型库覆盖、Phase 4 的更丰富逐视角可见实例证据，以及 Phase 5 的真实 AI backend 准入。当前系统已经能把确定性质量、显式语义/材质复核和最终交付包绑定到同一 active run；复核层是证据入口，不会调用云模型、猜目录或修补 CAD 阶段缺失。
...
| 1 | 推送第五批并清理 worktree | Phase 2 | 主线已通过复验，需推送远端并释放 `.worktrees/common-model-library-batch-5` | 远端 `main` 更新，临时分支和 worktree 不继续占用 |
| 2 | 模型质量报告用户化 | Phase 2 -> Phase 6 | 普通用户需要知道哪些零件是真 STEP、哪些是 B/C/D 级替代，才能判断照片级风险 | 项目向导/交付包能按零件展示真实模型、库模板、fallback 与需复核项 |
| 3 | 新用户项目入口再简化 | Phase 1 -> Phase 6 | 把全流程串成少提问、多确认的项目向导 | 用户只说产品和目标，系统按 Phase 给下一步 |
| 4 | 真实 AI backend adapter 准入 | Phase 5 | `gpt-image-2-pro` 等新后端必须走配置隔离、白名单、同 run 验收和多视角一致性测试 | 云增强可以作为可选 preset 接入，而不是手填 URL/key 执行 |
...
1. Phase 2：推送第五批常用模型库，并清理 `.worktrees/common-model-library-batch-5` 与临时分支。
2. Phase 2 -> Phase 6：把模型质量等级、来源和复核建议做成普通用户可读报告，避免用户只看到“出了图”却不知道哪些零件只是 B/C/D 级替代。
3. Phase 1 -> Phase 6：继续简化新用户项目入口，把全流程串成少提问、多确认的项目向导。
4. Phase 5：若接入 `gpt-image-2-pro`，先按 backend adapter 准入做配置隔离、白名单、同 run 验收和多视角一致性测试。
