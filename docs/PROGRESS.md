...
| --- | --- | --- | --- | --- |
| Phase 1 SPEC | 从设计文档抽取结构化 CAD 规格，并做设计审查 | `CAD_SPEC.md`、`DESIGN_REVIEW.json`、补充参数、入口 `PROJECT_GUIDE.json` | 87% | 已有结构化章节、审查门禁、模型选择补充和设计文档入口确认；下一步是把产品目标自然语言入口和补充参数回填做得更稳 |
| Phase 2 CODEGEN | 从 Spec 生成 CadQuery 代码、BOM 路由和标准件模型 | `params.py`、`build_all.py`、`assembly.py`、`std_*.py` | 86% | 模型库、resolver、五批常用模型族、准入清单和普通用户模型质量摘要已成型；仍需继续扩展跨产品高频件 |
...
| Phase 4 RENDER | 用 Blender 渲染多视角 3D 图，并绑定当前 run | `render_manifest.json`、多视角 `V*.png`、`RENDER_VISUAL_REGRESSION.json`、`RENDER_QUALITY_REPORT.json` | 82% | 路径契约、当前 run 绑定、契约层视觉/元件一致性门禁、Blender 预检和截图像素质量证据已建立；后续要补更丰富的逐视角可见实例和材质/灯光策略 |
| Phase 5 ENHANCE | 把 CAD 渲染图增强到照片级，并做一致性验收 | `*_enhanced.*`、`ENHANCEMENT_REPORT.json`、`ENHANCEMENT_REVIEW_REPORT.json`、provider preset | 86% | provider 选择、增强闭环、同 run 一致性验收、确定性 `quality_summary` 和显式语义/材质复核证据已接入；后续是实际 AI backend adapter 的准入和多视角策略 |
| Phase 6 ANNOTATE / DELIVER | 标注、交付最终图片包和证据摘要 | `*_labeled_*.png`、`DELIVERY_PACKAGE.json`、`semantic_material_review`、`model_quality_summary`、可读摘要 | 67% | `photo3d-deliver` 已生成 active-run 最终包，并可要求增强状态、`quality_summary`、`semantic_material_review` 和模型质量摘要同 run 绑定；后续仍需最终报告美化和缩略图/可视化验收 |

**总体进展：约 86%。** 最大剩余缺口不在单个零件，而在 Phase 1 的自然语言项目入口和补充参数回填、Phase 2 的跨产品模型库覆盖、Phase 4 的更丰富逐视角可见实例证据，以及 Phase 5 的真实 AI backend 准入。当前系统已经能把确定性质量、显式语义/材质复核、模型质量摘要和最终交付包绑定到同一 active run；复核层是证据入口，不会调用云模型、猜目录或修补 CAD 阶段缺失。
...
- `project-guide --from-design-doc --design-doc <path>` 已成为新用户入口：只读取一个显式设计文档，写 `.cad-spec-gen/project-guide/PROJECT_GUIDE.json`，返回 `needs_subsystem_confirmation` 和 `confirm_subsystem`，只给候选子系统与预览命令，必须用户确认后才进入子系统流程。

剩余：
- 把 `project-guide` 的新用户入口继续前移到“产品目标 + 设计文档”的自然语言确认流，减少补充参数的手工输入。
...
- 常用模型库五批扩展，以及“新增模型族必须有 positive/negative/route/precedence/dimension/geometry 测试”的准入清单。
- 第五批新增方形法兰伺服电机、行星减速机、拖链段、DIN 导轨继电器模块、按钮/操作盒；宽泛词如标签、图纸、手册、PDF 会落到 `other`，避免误触发默认模型。
- 真实 STEP、用户 STEP、SolidWorks/Toolbox、bd_warehouse、PartCAD 和 fallback 的优先级边界。
- `model-audit --json` 已输出 `model_quality_summary`：按零件展示 model source、A-E 质量、真实 STEP/模型库模板/fallback 分类、`readiness_status`、`photoreal_risk`、阻断项和复核动作。
- `project-guide` 只读 `cad/<subsystem>/.cad-spec-gen/geometry_report.json` 并嵌入同一 `model_quality_summary`；不调用 resolver、不扫描输出目录、不改变下一步命令。

剩余：
- 继续扩展跨产品高频件，但按准入清单做，不再按某个设备临时收紧。
- 把更多高频真实/参数化模型源纳入默认库或可配置库，减少 C/D/E fallback。
...
- `render-visual-check` 只读取 `ARTIFACT_INDEX.json.active_run_id`，写 `RENDER_VISUAL_REGRESSION.json`，检查当前 run 的视角、渲染文件、哈希链、装配实例和 accepted baseline；有逐视角实例证据时会比较可见实例集合，缺少证据时只给 warning，不冒充图片身份已被证明。
- `render-quality-check` 只读取 `ARTIFACT_INDEX.json.active_run_id` 和同 run `render_manifest.json`，写 `RENDER_QUALITY_REPORT.json`；检查 `blender_preflight`、渲染文件路径/hash/基础 QA，并计算逐视角 `pixel_metrics`。
- manifest `run_id` 漂移、render_dir/path/hash 漂移、缺 Blender 或基础 QA 失败会阻断；低对比度、边缘密度低、画布不一致只给 warning，便于普通用户继续决策。

剩余：
- 让 render manifest 生成更丰富的逐视角可见实例证据，减少只能 warning 的情况。
- 继续补视角/灯光/材质策略检查和更接近人眼判断的画面质量复核。
...
- `enhance-check` 已为每个成功增强视角写入 `quality_metrics`，并在 `ENHANCEMENT_REPORT.json.quality_summary` 汇总画布尺寸、对象占用率、亮度、对比度和饱和度等确定性质量证据。
- 低对比度或多视角画布尺寸不一致会把增强报告降为 `preview`，不冒充照片级最终交付；缺视角、路径越界、同视角多候选等输入问题仍保持 `blocked`。
- `enhance-review` 已接入显式人工/大模型复核证据，读取 `--review-input`，校验 active run、`render_manifest.json`、`ENHANCEMENT_REPORT.json` 路径和 sha256，并输出 `ENHANCEMENT_REVIEW_REPORT.json.semantic_material_review`。
- 复核检查项是通用的 `geometry_preserved`、`material_consistent`、`photorealistic`、`no_extra_parts`、`no_missing_parts`；状态为 `accepted` / `preview` / `needs_review` / `blocked`，不调用 AI、不接受 backend/model/key/url、不扫描目录。

剩余：
- 真实 AI 复核或增强 backend 需要作为独立 adapter 进入白名单，不能把 URL/key/model 直接塞进 `enhance-review`。
- 若要加入 `gpt-image-2-pro`，必须先做真实 backend adapter、配置隔离、一致性验收和安全测试，再进入白名单。

### Phase 6 ANNOTATE / DELIVER：67%
...
- `photo3d-deliver` 已把当前 active run 的源渲染、增强图、可唯一识别的标注图、`ENHANCEMENT_REPORT.json`、`render_manifest.json`、`PHOTO3D_RUN.json` 和契约证据打包到 `delivery/DELIVERY_PACKAGE.json` 与 `README.md`。
- 默认只有 `accepted` 才复制最终图片并标记 `final_deliverable`；`preview` / `blocked` 只写证据报告，不作为照片级最终交付。
- `DELIVERY_PACKAGE.json` 已复制 `quality_summary`；如果增强状态 accepted 但质量摘要不是 accepted，交付会记录 `photo_quality_not_accepted` 并保持 `final_deliverable=false`。
- `photo3d-deliver` 已接入 `ENHANCEMENT_REVIEW_REPORT.json`：同一 run 若已有 `semantic_material_review` 且不是 accepted，会记录 `semantic_review_not_accepted` 并阻止最终交付；传 `--require-semantic-review` 时缺少 accepted 复核会记录 `semantic_review_required`。
- `photo3d-deliver` 已从同一 active run 的 `MODEL_CONTRACT.json` 派生 `model_quality_summary`，并把 `MODEL_CONTRACT.json` 作为证据复制进交付包；最终包不会引用可能过期的项目级 `geometry_report.json`。

剩余：
- 让交付摘要更适合普通用户直接阅读，并补可视化验收截图/缩略图。
- 在项目向导里把“已可交付 / 只能预览 / 被阻断”的原因讲清楚。
- 把最终报告做得更直观，包括缩略图、复核摘要和下一步建议。
...
| Done | 最终交付包 | Phase 6 | 照片级结果需要可交付，不只是生成一张图 | `delivery/` 目录里有增强图、标注图、证据报告和用户摘要 |
| Done | Blender 视觉回归和元件一致性检查（契约层） | Phase 4 | 防止出现“新渲染比旧渲染少元件”的通用问题 | `RENDER_VISUAL_REGRESSION.json` 报告元件数量/身份/视角证据是否漂移 |
| Done | 多视角照片级质量验收（确定性契约层） | Phase 5 -> Phase 6 | 防止增强图虽然完整但画面低对比度、画布不一致仍被当作最终交付 | `ENHANCEMENT_REPORT.json.quality_summary` / `DELIVERY_PACKAGE.json.quality_summary` 和 `photo_quality_not_accepted` |
| Done | Blender 环境预检和截图级回归（确定性像素层） | Phase 4 | 契约层能防少件，还需要证明 Blender 环境、画面和像素质量稳定 | `RENDER_QUALITY_REPORT.json` 报告 Blender 预检、路径/hash/QA 和逐视角像素指标 |
| Done | 语义/材质级增强质量复核 | Phase 5 -> Phase 6 | 当前质量门禁是确定性指标，仍不能替代语义结构、材质真实感和人眼审美判断 | `ENHANCEMENT_REVIEW_REPORT.json.semantic_material_review` 和 `photo3d-deliver --require-semantic-review` |
| Done | 常用模型库第五批 | Phase 2 | 继续提高不同产品零配置成图质量，但所有新增族必须通过准入清单 | 方形法兰伺服电机、行星减速机、拖链段、DIN 继电器模块、按钮/操作盒自动走可辨识 B 级模板 |
| Done | 提交/合并第五批 | Phase 2 | 代码和文档门禁已通过，并已进入主线 | `main` 已拿到第五批模型库能力 |
| Done | 模型质量报告用户化 | Phase 2 -> Phase 6 | 普通用户需要知道哪些零件是真 STEP、哪些是 B/C/D 级替代，才能判断照片级风险 | `model-audit`、`project-guide` 和 `photo3d-deliver` 都能按零件展示真实模型、库模板、fallback、阻断项与复核动作 |
| Done | 新用户项目入口再简化（设计文档入口） | Phase 1 -> Phase 6 | 先完成“只有设计文档也能启动”的确认边界，避免目录扫描和猜子系统 | `.cad-spec-gen/project-guide/PROJECT_GUIDE.json` 输出 `needs_subsystem_confirmation`，用户确认子系统后才继续 |
| 1 | 新用户项目入口继续前移 | Phase 1 -> Phase 6 | 把“产品目标 + 设计文档 + 缺失参数”串成更少提问、多确认的项目向导 | 用户只说产品和目标，系统按 Phase 给下一步，并解释当前缺什么 |
| 2 | 真实 AI backend adapter 准入 | Phase 5 | `gpt-image-2-pro` 等新后端必须走配置隔离、白名单、同 run 验收和多视角一致性测试 | 云增强可以作为可选 preset 接入，而不是手填 URL/key 执行 |
| 3 | 逐视角可见实例证据增强 | Phase 4 -> Phase 6 | 契约层已经能防少件，但逐视角实例证据越丰富，普通用户越容易判断图是否完整 | 渲染/交付报告能展示每个视角看到哪些实例，减少只能 warning 的情况 |
| 4 | 最终交付报告可视化 | Phase 6 | 交付包已有证据 JSON，普通用户还需要更直观的摘要和缩略图 | `delivery/README.md` 更像验收页：缩略图、质量摘要、复核状态、下一步动作 |
...
- `enhance-check` 会写 `ENHANCEMENT_REPORT.json.quality_summary` 和逐视角 `quality_metrics`；当前确定性质量指标覆盖画布尺寸、对象占用率、亮度、对比度和饱和度，不等同于语义 AI 审美或人工照片级评审。
- `ENHANCEMENT_REPORT.json` 的 `delivery_status == accepted` 且 `quality_summary.status == accepted` 才能作为最终交付；`preview` 只能预览，`blocked` 表示缺视角、路径越界、同视角多候选或输入不完整。
- `enhance-review` 只读取显式 `--review-input` JSON，并绑定 `ARTIFACT_INDEX.json.active_run_id`、同 run `render_manifest.json`、同 run `ENHANCEMENT_REPORT.json` 的路径和 sha256；它不调用 AI、不接受 backend/model/key/url、不扫描目录、不用复核结果修补 CAD 几何。
- `ENHANCEMENT_REVIEW_REPORT.json.semantic_material_review` 是照片级语义/材质复核证据，状态为 `accepted` / `preview` / `needs_review` / `blocked`；缺少视角、额外视角、重复视角、source report 路径/hash 漂移会 `blocked`，检查项缺失会 `needs_review`，检查项为 false 会 `preview`。
- `photo3d-deliver` 只读取当前 `ARTIFACT_INDEX.json.active_run_id` 的固定证据链，默认输出 `cad/<subsystem>/.cad-spec-gen/runs/<run_id>/delivery/DELIVERY_PACKAGE.json` 和 `README.md`；它不扫描 render 目录猜最新图，不换 run，不接受 subsystem/run_id/render_manifest 漂移。
- `photo3d-deliver` 默认只有增强验收 `accepted` 且 `quality_summary.status == accepted` 才复制最终增强图、源图和可唯一识别的标注图，并标记 `final_deliverable=true`；质量未通过会写 `photo_quality_not_accepted`，`preview` / `blocked` 只写证据报告，除非显式 `--include-preview` 才复制预览图，且 `final_deliverable` 仍为 false。
- `photo3d-deliver --require-semantic-review` 要求同一 active run 的 `semantic_material_review.status == accepted` 才能复制最终图；即使不强制，只要同一 run 已有非 accepted 复核报告，也会记录 `semantic_review_not_accepted` 并阻止最终交付。
- `render-visual-check` 只读取当前 `ARTIFACT_INDEX.json.active_run_id` 的 `PRODUCT_GRAPH.json`、`ASSEMBLY_SIGNATURE.json`、`render_manifest.json` 和 accepted baseline 证据，输出当前 run 目录下的 `RENDER_VISUAL_REGRESSION.json`；它不扫描 render 目录猜最新图、不切换 run、不接受 render dir/path/hash 漂移。
- `render-visual-check` 会阻断当前 run 缺视角、重复视角、渲染文件丢失/越界/哈希不匹配、产品必需实例未进入装配签名、相对 accepted baseline 少视角或少装配实例；如果 baseline 和当前 manifest 都有逐视角实例证据，还会阻断可见实例证据丢失。
- `render-visual-check` 是契约层和证据层门禁，不是像素级/语义分割视觉识别；没有逐视角实例证据时只能给 `warning`，不能宣称图片内每个元件身份都已被视觉证明。
- `render-quality-check` 只读取当前 `ARTIFACT_INDEX.json.active_run_id` 和同 run `render_manifest.json`，输出当前 run 目录下的 `RENDER_QUALITY_REPORT.json`；它不扫描 render 目录猜最新图、不切换 run、不允许 `render_manifest.json.run_id` 覆盖 `active_run_id`。
- `render-quality-check` 会阻断缺 Blender、Blender 版本预检失败、manifest 子系统/run 漂移、render_dir 不属于当前 run、渲染文件缺失/越界/hash 漂移和基础 QA 失败；低对比度、边缘密度低或多视角画布不一致是 `warning`。
- `RENDER_QUALITY_REPORT.json.pixel_metrics` 是确定性截图像素证据，覆盖画布尺寸、主体占比、亮度、对比度、饱和度和边缘密度；它不是语义 AI 判断，也不能证明某个元件身份。
...
- `project-guide --from-design-doc` 是子系统选择前的入口模式；它只读取一个显式设计文档，输出固定在 `.cad-spec-gen/project-guide/PROJECT_GUIDE.json`，状态只能先到 `needs_subsystem_confirmation`，不会扫描 `docs/design`/`cad`、不会猜最新 run、不会初始化子系统、不会生成 spec/codegen/build/render/enhance 产物。
...
- `model_quality_summary` 是从既有事实派生的普通用户视图，不是新的几何事实源；`quality_counts`、`source_counts`、`readiness_status`、`photoreal_risk` 和 `part_summaries` 只能解释模型质量，不能覆盖 `geometry_report.json` 或 `MODEL_CONTRACT.json` 的原始结论。
- `model-audit --json` 的 `model_quality_summary` 只基于显式输入的 `geometry_report.json` 派生；保留原有 `status` 兼容行为，不为了普通用户文案降低 `missing_step` 或 `review_required` 的严格性。
- `project-guide` 只从规范路径 `cad/<subsystem>/.cad-spec-gen/geometry_report.json` 读取项目级 `model_quality_summary`；缺报告时写 `not_available`，不调用 resolver、不扫描输出目录、不猜某个零件应该用什么模型。
- `photo3d-deliver` 的 `model_quality_summary` 只从同一 active run 的 `MODEL_CONTRACT.json` 派生，避免最终交付包误引用过期项目级 `geometry_report.json`；模型质量未就绪时仍只作为交付风险和复核动作，不让 AI 增强补 CAD 缺件。
...
1. Phase 1 -> Phase 6：继续把新用户项目入口从“显式设计文档入口”前移到“产品目标 + 设计文档 + 缺失参数”的少提问、多确认向导。
2. Phase 5：若接入 `gpt-image-2-pro`，先做真实 backend adapter 准入，包含配置隔离、白名单、同 run 验收、多视角一致性和密钥不落盘测试。
3. Phase 4 -> Phase 6：增强逐视角可见实例证据，让“这张图有没有少零件”从装配契约进一步走向视角级证据。
4. Phase 6：把最终交付包 README 做成更直观的验收页，补缩略图、模型质量摘要、语义/材质复核状态和下一步动作。
...
| 2026-05-06 | `python -m pytest tests\test_project_guide.py -q` / `python -m pytest tests\test_photo3d_user_flow.py -q` | 新用户入口核心套件通过：`16 passed, 1 warning`；Photo3D 用户流通过：`14 passed, 1 warning` |
| 2026-05-06 | `python -m pytest tests\test_project_guide.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` | 新用户入口组合回归 `172 passed, 1 warning` |
| 2026-05-06 | `python -m pytest tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` / `git diff --check` | 看板/README 收尾后 `142 passed, 1 warning`；空白检查通过，仅 Windows 行尾提示 |
| 2026-05-06 | `git merge --ff-only codex/new-user-entry-guide` | 功能提交 `0ac375a` 已快进合并到本地 `main` |
| 2026-05-06 | `python scripts\dev_sync.py --check` / `python -m pytest tests\test_project_guide.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` / `git diff --check` | 合并到本地 `main` 后复验通过：`172 passed, 1 warning`，同步无漂移，空白检查通过 |
| 2026-05-06 | `git push origin main` | 推送失败：GitHub 443 无法连接；本地 `main` 已保留提交，待网络恢复后重试 |
| 2026-05-06 | `git push origin main` | 网络恢复后推送成功：`faf3246..397f733 main -> main` |
| 2026-05-06 | `python -m pytest tests\test_model_audit_cli.py -q` | 模型质量摘要红绿后通过：`6 passed`；`model-audit --json` 已输出普通用户可读 `model_quality_summary` |
| 2026-05-06 | `python -m pytest tests\test_model_audit_cli.py::test_model_audit_summary_normalizes_unknown_quality_counts -q` | 先红后绿，修正 `model_quality_summary.quality_counts` 与逐零件 `geometry_quality` 的 unknown 归一化一致性 |
| 2026-05-06 | `python -m pytest tests\test_model_audit_cli.py tests\test_project_guide.py -q` | `18 passed`；覆盖 `project-guide` 只读嵌入规范路径 `geometry_report.json` 的模型质量摘要 |
| 2026-05-06 | `python -m pytest tests\test_photo3d_delivery_pack.py -q` | `7 passed`；覆盖最终交付包从 active-run `MODEL_CONTRACT.json` 派生 `model_quality_summary` |
| 2026-05-06 | `python -m pytest tests\test_photo3d_user_flow.py -q` | `14 passed`；覆盖 CLI help、cad-help 文档、metadata 和安装版镜像对模型质量摘要的描述 |
| 2026-05-06 | `python scripts\dev_sync.py` / `python scripts\dev_sync.py --check` | 同步 7 个安装版镜像：`cad_pipeline.py`、3 个 tools、`cad-help.md`、`skill_cad_help_zh.md`、`skill.json`；随后检查通过 |
| 2026-05-06 | `python -m pytest tests\test_model_audit_cli.py tests\test_project_guide.py tests\test_photo3d_delivery_pack.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` | 本轮核心范围 `182 passed, 1 warning`；warning 为既有 pytest `env` 配置提示 |
| 2026-05-06 | `python -m pytest tests\test_model_contract.py tests\test_photo3d_gate_contract.py tests\test_photo3d_gate_matrix.py -q` | 模型契约和 Photo3D 门禁周边回归 `19 passed, 1 warning` |
| 2026-05-06 | `git diff --check` | 空白检查通过；仅 Windows 行尾提示 |
| 2026-05-06 | `git merge --ff-only codex/model-quality-user-report` | 功能提交 `2ab8da2` 已快进合并到本地 `main` |
| 2026-05-06 | `python scripts\dev_sync.py --check` / `python -m pytest tests\test_model_audit_cli.py tests\test_project_guide.py tests\test_photo3d_delivery_pack.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` / `git diff --check` | 合并到本地 `main` 后复验通过：`182 passed, 1 warning`，同步无漂移，空白检查通过 |
| 2026-05-06 | `python -m pytest tests\test_model_contract.py tests\test_photo3d_gate_contract.py tests\test_photo3d_gate_matrix.py -q` | 合并到本地 `main` 后周边回归 `19 passed, 1 warning` |
| 2026-05-06 | `git push origin main` | 两次推送失败：GitHub 443 连接被重置/无法连接；本地 `main` 已保留提交，待网络恢复后重试 |
| 2026-05-05 | `python -m pytest tests\test_common_model_library_batch_5.py -q` | 第五批常用模型库目标测试红绿后通过：`34 passed`；覆盖分类、负例、B 级模板 metadata、默认路由、category-scoped 尺寸和几何包络 |
| 2026-05-05 | `python -m pytest tests\test_common_model_library_batch_5.py tests\test_parts_library_standard_categories.py tests\test_common_model_family_admission.py -q` | 第五批准入和标准类别范围 `85 passed`；确认默认库路由、准入 manifest 和通用负例一致 |
| 2026-05-05 | `python -m pytest tests\test_common_model_library_batch_5.py tests\test_common_model_family_admission.py tests\test_common_model_library_batch_4.py tests\test_common_model_library_batch_3.py tests\test_common_model_library_batch_2.py tests\test_common_model_library_expansion.py tests\test_parts_library_standard_categories.py tests\test_jinja_generators_new.py -q` | 第五批扩展范围回归 `330 passed, 7 warnings` |
| 2026-05-05 | `python scripts\dev_sync.py` / `python scripts\dev_sync.py --check` | 同步 3 个本地安装镜像：`bom_parser.py`、`cad_spec_defaults.py`、`parts_library.default.yaml`；随后检查通过 |
| 2026-05-05 | `python -m pytest tests\test_common_model_library_batch_5.py tests\test_common_model_family_admission.py tests\test_common_model_library_batch_4.py tests\test_common_model_library_batch_3.py tests\test_common_model_library_batch_2.py tests\test_common_model_library_expansion.py tests\test_parts_library_standard_categories.py tests\test_parts_adapters.py tests\test_jinja_generators_new.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` / `git diff --check` | 第五批最终范围门禁 `514 passed, 2 skipped, 11 warnings`；空白检查通过，仅 Windows 行尾提示 |
| 2026-05-05 | 合并前代码审查 | 发现并修复 2 个 P1：公开 route 别名会拿到 category fallback 错误尺寸，`interface relay` 会路由到默认库但 adapter 不进继电器模块模板 |
| 2026-05-05 | `python -m pytest tests\test_common_model_library_batch_5.py tests\test_parts_library_standard_categories.py -q` | 别名一致性红绿验证：先 14 failed 复现 review 问题，修正后 `99 passed, 7 warnings` |
| 2026-05-05 | `python -m pytest tests\test_common_model_library_batch_5.py tests\test_parts_library_standard_categories.py tests\test_common_model_family_admission.py -q` | 审查修正后核心准入范围 `108 passed, 7 warnings`；准入 manifest 现在会检查 route 别名的 template、real_dims 和 lookup dims 一致性 |
| 2026-05-05 | `python -m pytest tests\test_common_model_library_batch_5.py tests\test_common_model_family_admission.py tests\test_common_model_library_batch_4.py tests\test_common_model_library_batch_3.py tests\test_common_model_library_batch_2.py tests\test_common_model_library_expansion.py tests\test_parts_library_standard_categories.py tests\test_jinja_generators_new.py -q` | 审查修正后扩展范围回归 `353 passed, 7 warnings` |
| 2026-05-05 | `python scripts\dev_sync.py` / `python scripts\dev_sync.py --check` | 审查修正后同步 1 个本地安装镜像：`cad_spec_defaults.py`；随后检查通过 |
| 2026-05-05 | `python -m pytest tests\test_common_model_library_batch_5.py tests\test_common_model_family_admission.py tests\test_common_model_library_batch_4.py tests\test_common_model_library_batch_3.py tests\test_common_model_library_batch_2.py tests\test_common_model_library_expansion.py tests\test_parts_library_standard_categories.py tests\test_parts_adapters.py tests\test_jinja_generators_new.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` / `git diff --check` | 审查修正后最终范围门禁 `537 passed, 2 skipped, 11 warnings`；空白检查通过，仅 Windows 行尾提示 |
| 2026-05-05 | `git merge --ff-only codex/common-model-library-batch-5` | 第五批功能提交 `86948c3` 已快进合并到 `main` |
| 2026-05-05 | `python scripts\dev_sync.py` / `python scripts\dev_sync.py --check` | 合并到 `main` 后同步 3 个本地安装镜像，随后检查通过 |
| 2026-05-05 | `python -m pytest tests\test_common_model_library_batch_5.py tests\test_common_model_family_admission.py tests\test_common_model_library_batch_4.py tests\test_common_model_library_batch_3.py tests\test_common_model_library_batch_2.py tests\test_common_model_library_expansion.py tests\test_parts_library_standard_categories.py tests\test_parts_adapters.py tests\test_jinja_generators_new.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` / `git diff --check` | 合并到 `main` 后最终范围复验 `537 passed, 2 skipped, 11 warnings`；空白检查通过 |
| 2026-05-05 | `python -m pytest tests\test_render_quality_check.py::test_render_quality_check_blocks_manifest_run_drift_and_writes_active_run_report -q` | 先红后绿，确认 `render_manifest.json.run_id` 漂移不能覆盖 `ARTIFACT_INDEX.json.active_run_id`，报告仍写回当前 run |
| 2026-05-05 | `python -m pytest tests\test_render_quality_check.py::test_render_quality_check_blocks_empty_render_manifest_files -q` | 先红后绿，确认空 `render_manifest.files` 不会误判 pass，而是 `render_manifest_empty` blocked |
| 2026-05-05 | `python -m pytest tests\test_render_quality_check.py -q` | Phase 4 渲染质量门禁目标测试 `9 passed`；覆盖 Blender 预检、hash 漂移、run/render_dir 漂移、文件越界、低对比度 warning、缺 render_dir 和空 manifest blocked |
| 2026-05-05 | `python scripts\dev_sync.py --check` / `python -m pytest tests\test_render_quality_check.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` | 同步、CLI/docs/metadata/packaging 核心范围 `163 passed`，安装版镜像无漂移 |
| 2026-05-05 | `python -m pytest tests\test_render_quality_check.py tests\test_render_visual_regression.py tests\test_render_qa.py tests\test_render_manifest_signature.py tests\test_photo3d_gate_contract.py tests\test_photo3d_gate_matrix.py tests\test_photo3d_path_drift.py tests\test_photo3d_stale_artifacts.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` / `git diff --check` | Phase 4/Photo3D 周边回归 `199 passed`；空白检查通过，仅 Windows 行尾提示 |
| 2026-05-05 | `git merge --ff-only codex/blender-preflight-visual-qa` | Phase 4 渲染质量预检功能提交 `e051e37` 已快进合并到 `main` |
| 2026-05-05 | `python scripts\dev_sync.py --check` / `python -m pytest tests\test_render_quality_check.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` / `git diff --check` | 合并到 `main` 后复验通过：`163 passed`，安装版镜像无漂移，空白检查通过 |
| 2026-05-05 | `python -m pytest tests\test_render_quality_check.py tests\test_render_visual_regression.py tests\test_render_qa.py tests\test_render_manifest_signature.py tests\test_photo3d_gate_contract.py tests\test_photo3d_gate_matrix.py tests\test_photo3d_path_drift.py tests\test_photo3d_stale_artifacts.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` | 合并到 `main` 后 Phase 4/Photo3D 周边回归 `199 passed` |
| 2026-05-05 | `python -m pytest tests\test_enhance_consistency.py -q` | 多视角质量摘要红绿后通过：`13 passed`；覆盖 accepted 报告、低对比度 preview、画布尺寸不一致 preview 和逐视角 `quality_metrics` |
| 2026-05-05 | `python -m pytest tests\test_photo3d_delivery_pack.py -q` | 交付质量门禁红绿后通过：`6 passed`；覆盖 `DELIVERY_PACKAGE.json.quality_summary` 和 `photo_quality_not_accepted` 阻断最终交付 |
| 2026-05-05 | `python -m pytest tests\test_photo3d_user_flow.py -q` | 文档/CLI/metadata 红测曾因缺 `quality_summary` / `photo_quality_not_accepted` 描述失败；更新后通过 |
| 2026-05-05 | `python -m pytest tests\test_enhance_consistency.py tests\test_photo3d_delivery_pack.py tests\test_photo3d_user_flow.py -q` | 本轮核心范围 `31 passed` |
| 2026-05-05 | `python -m pytest tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` / `python scripts\dev_sync.py --check` | 打包同步、安装版镜像和数据目录同步 `140 passed`；同步检查通过 |
| 2026-05-05 | `python scripts\dev_sync.py --check` / `python -m pytest tests\test_enhance_consistency.py tests\test_photo3d_delivery_pack.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` | 提交前范围复验通过：`171 passed`，安装版镜像无漂移 |
| 2026-05-05 | `python -m pytest tests\test_photo3d_handoff.py tests\test_enhance_consistency.py tests\test_photo3d_autopilot.py tests\test_photo3d_loop.py tests\test_project_guide.py tests\test_photo3d_delivery_pack.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` / `git diff --check` | Photo3D 周边回归 `218 passed`；空白检查通过，仅 Windows 行尾提示 |
| 2026-05-05 | `git merge --ff-only codex/photo3d-quality-gate` | 多视角照片级质量门禁功能提交 `4963a8c` 已快进合并到 `main` |
| 2026-05-05 | `python scripts\dev_sync.py --check` / `python -m pytest tests\test_enhance_consistency.py tests\test_photo3d_delivery_pack.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` / `git diff --check` | 合并到 `main` 后复验通过：`171 passed`，安装版镜像无漂移，空白检查通过 |
| 2026-05-05 | `python -m pytest tests\test_render_visual_regression.py -q` | Phase 4 渲染视觉/元件一致性门禁目标测试 `8 passed` |
| 2026-05-05 | `python scripts\dev_sync.py --check` / `python -m pytest tests\test_render_visual_regression.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py -q` | 本轮文档/metadata/安装版镜像同步后，视觉回归和用户流范围 `23 passed` |
| 2026-05-05 | `python scripts\dev_sync.py --check` / `python -m pytest tests\test_render_visual_regression.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` | 收尾同步与核心范围回归通过：`160 passed` |
| 2026-05-05 | `python -m pytest tests\test_render_manifest_signature.py tests\test_render_qa.py tests\test_photo3d_gate_contract.py tests\test_photo3d_gate_matrix.py tests\test_photo3d_path_drift.py tests\test_photo3d_stale_artifacts.py -q` / `git diff --check` | Phase 4/Photo3D 周边回归 `28 passed`；空白检查通过，仅 Windows 行尾提示 |
| 2026-05-05 | `git merge --ff-only codex/render-visual-regression` | Phase 4 渲染视觉一致性门禁功能提交 `769873b` 已快进合并到 `main` |
| 2026-05-05 | `python scripts\dev_sync.py --check` / `python -m pytest tests\test_render_visual_regression.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` | 合并后首次复查发现新增工具镜像被 `.gitignore` 忽略未纳入提交，`dev_sync --check` 和打包同步测试按预期阻断 |
| 2026-05-05 | `python scripts\dev_sync.py` / `git add -f src\cad_spec_gen\data\tools\render_visual_regression.py` / `git commit -m "chore(sync): 纳入渲染视觉门禁镜像"` | 已生成并显式纳入安装版工具镜像，修正提交 `9003bef` |
| 2026-05-05 | `python scripts\dev_sync.py --check` / `python -m pytest tests\test_render_visual_regression.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` / `git diff --check` | 镜像修正后主线复查通过：`160 passed`，空白检查通过 |
...
| 2026-05-05 | `git worktree add .worktrees\photo3d-delivery-pack -b codex/photo3d-delivery-pack` / `python scripts\dev_sync.py` / `python scripts\dev_sync.py --check` / `python -m pytest tests\test_photo3d_handoff.py tests\test_enhance_consistency.py tests\test_photo3d_loop.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` | 新 worktree ignored mirror 填充后基线通过：`173 passed` |
| 2026-05-05 | `python -m pytest tests\test_photo3d_delivery_pack.py -q` | 红测阶段缺 `tools.photo3d_delivery_pack` / `cmd_photo3d_deliver`，并补充旧交付图片清理回归；实现后 `5 passed` |
| 2026-05-05 | `python -m pytest tests\test_photo3d_user_flow.py::test_photo3d_deliver_help_explains_final_delivery_package tests\test_photo3d_user_flow.py::test_cad_help_docs_describe_photo3d_foolproof_user_flow tests\test_photo3d_user_flow.py::test_skill_metadata_advertises_photo3d_and_llm_action_reports tests\test_photo3d_packaging_sync.py::test_photo3d_contract_tools_have_packaged_mirrors -q` | 文档/metadata/安装版镜像红绿测试最终 `4 passed` |
| 2026-05-05 | `python scripts\dev_sync.py --check` / `python -m pytest tests\test_photo3d_delivery_pack.py tests\test_photo3d_handoff.py tests\test_enhance_consistency.py tests\test_photo3d_autopilot.py tests\test_photo3d_loop.py tests\test_project_guide.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` / `git diff --check` | 最终交付包范围回归 `213 passed`，无同步漂移，空白检查通过 |
| 2026-05-05 | `git merge --ff-only codex/photo3d-delivery-pack` | 最终交付包功能提交 `3baff13` 已快进合并到 `main` |
| 2026-05-05 | `git push origin main` / `git worktree remove .worktrees\photo3d-delivery-pack` / `git branch -d codex/photo3d-delivery-pack` / `git worktree prune` | `main` 已推送到 `c46d461`；本轮 worktree/分支已清理，旧独立 worktree `.worktrees/generic-threaded-photo-autopilot` 未触碰 |
