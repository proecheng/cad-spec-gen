...
| --- | --- |
| 更新日期 | 2026-05-05 |
| 当前主线 | `main` 已快进合并本轮 Phase 5 -> Phase 6 多视角照片级质量门禁；待最终推送和清理 `codex/photo3d-quality-gate` worktree/分支 |
| 管线 Phase 数 | 6 个：SPEC / CODEGEN / BUILD / RENDER / ENHANCE / ANNOTATE |
| 总体能力进展 | 约 80%（按 6 个 Phase 的工程化能力估算，不代表某个具体产品一次出图进度） |
| 当前主攻 Phase | Phase 5 ENHANCE -> Phase 6 DELIVER：`enhance-check` 已新增确定性 `quality_summary`，`photo3d-deliver` 已接入最终交付质量门禁 |
| 最新功能基线 | 多视角照片级质量门禁：`ENHANCEMENT_REPORT.json.quality_summary`、逐视角 `quality_metrics`、`DELIVERY_PACKAGE.json.quality_summary` 和 `photo_quality_not_accepted` 阻断原因 |
| 最新合并/进度提交 | 功能提交 `4963a8c feat(photo3d): 增加多视角照片级质量验收` 已快进合并到 `main`；上一轮 Phase 4 视觉门禁为 `769873b` + `9003bef` |
| 最新归档计划提交 | `9ed3280 docs(project): 归档通用传动件计划` |
| 最近验证 | `python -m pytest tests\test_enhance_consistency.py tests\test_photo3d_delivery_pack.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` -> `171 passed`；Photo3D 周边回归 `218 passed` |
| 同步检查 | `python scripts\dev_sync.py --check` -> 通过；`git diff --check` -> 通过（仅 Windows 行尾提示） |
| 当前未跟踪 | 无运行时产物纳入；本轮新增计划、工具、测试、文档和安装版镜像已随功能提交进入 `main`。另有独立旧 worktree `.worktrees/generic-threaded-photo-autopilot`，本轮不触碰 |

## 一句话结论

cad-spec-gen 已形成 6 阶段 CAD 混合渲染管线。现在不是围绕某个零件做临时调参，而是在把“其他用户、其他产品也能傻瓜式出照片级 3D 图”的通用流程补齐：本轮把 Phase 5/6 的增强验收和最终交付接上确定性照片级质量证据，增强图只有在状态和 `quality_summary` 都 accepted 时才会进入最终交付。
...
| Phase 4 RENDER | 用 Blender 渲染多视角 3D 图，并绑定当前 run | `render_manifest.json`、多视角 `V*.png`、`RENDER_VISUAL_REGRESSION.json` | 78% | 路径契约、当前 run 绑定和契约层视觉/元件一致性门禁已建立；后续要补 Blender 预检、像素/截图级回归和更稳定的材质/灯光策略 |
...
**总体进展：约 80%。** 最大剩余缺口不在单个零件，而在 Phase 4 的 Blender 画面级证据，以及 Phase 5/6 的语义/材质级照片级判断：当前质量门禁已经能基于画布、占用率、亮度、对比度、饱和度和多视角尺寸一致性阻断明显不可交付结果，但还不是 AI/人工语义审美裁决。
...
剩余：
- 加入更系统的 Blender 环境预检、视角/灯光/材质质量检查和截图/像素级回归。
...
| 1 | Blender 环境预检和截图级回归 | Phase 4 | 契约层能防少件，仍需要证明 Blender 环境、画面和材质质量稳定 | 渲染前能发现 Blender/材质/灯光问题，渲染后有像素或截图级质量证据 |
| 2 | 语义/材质级增强质量复核 | Phase 5 -> Phase 6 | 当前质量门禁是确定性指标，仍不能替代语义结构、材质真实感和人眼审美判断 | 引入可配置 AI/人工复核入口，但仍绑定 active run 和报告证据 |
...
1. Phase 4：补 Blender 环境预检、材质/灯光质量检查和截图/像素级回归，让契约层门禁之后还有画面质量证据。
2. Phase 5 -> Phase 6：在确定性 `quality_summary` 之上增加语义/材质级增强复核入口，仍保持 active run 和证据报告绑定。
3. Phase 2：按通用模型族准入清单继续扩展下一批常用件，不做单设备临时收紧。
4. Phase 1 -> Phase 6：继续简化新用户项目入口，把全流程串成少提问、多确认的项目向导。
