...
| --- | --- |
| 更新日期 | 2026-05-05 |
| 当前主线 | `main` 已快进合并本轮 Phase 4 Blender 预检和截图像素质量门禁；待最终推送和清理 `codex/blender-preflight-visual-qa` worktree/分支 |
| 管线 Phase 数 | 6 个：SPEC / CODEGEN / BUILD / RENDER / ENHANCE / ANNOTATE |
| 总体能力进展 | 约 81%（按 6 个 Phase 的工程化能力估算，不代表某个具体产品一次出图进度） |
| 当前主攻 Phase | Phase 4 RENDER：新增 `render-quality-check`，把 Blender 预检、渲染文件路径/hash/QA 和逐视角像素指标纳入 active-run 证据链 |
| 最新功能基线 | Phase 4 渲染质量门禁：`RENDER_QUALITY_REPORT.json`、`blender_preflight`、`render_quality_summary`、逐视角 `pixel_metrics`，并阻断 manifest run/path/hash 漂移 |
| 最新合并/进度提交 | 功能提交 `e051e37 feat(photo3d): 增加渲染质量预检` 已快进合并到 `main`；上一轮多视角照片级质量门禁为 `4963a8c`，上一轮 Phase 4 视觉门禁为 `769873b` + `9003bef` |
| 最新归档计划提交 | `9ed3280 docs(project): 归档通用传动件计划` |
| 最近验证 | `python -m pytest tests\test_render_quality_check.py tests\test_render_visual_regression.py tests\test_render_qa.py tests\test_render_manifest_signature.py tests\test_photo3d_gate_contract.py tests\test_photo3d_gate_matrix.py tests\test_photo3d_path_drift.py tests\test_photo3d_stale_artifacts.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` -> `199 passed` |
| 同步检查 | `python scripts\dev_sync.py --check` -> 通过；`git diff --check` -> 通过（仅 Windows 行尾提示） |
| 当前未跟踪 | 无运行时产物纳入；本轮新增工具、测试、文档和安装版镜像已随功能提交进入 `main`。另有独立旧 worktree `.worktrees/generic-threaded-photo-autopilot`，本轮不触碰 |

## 一句话结论

cad-spec-gen 已形成 6 阶段 CAD 混合渲染管线。现在不是围绕某个零件做临时调参，而是在把“其他用户、其他产品也能傻瓜式出照片级 3D 图”的通用流程补齐：本轮把 Phase 4 的 Blender 环境和截图像素质量接成 active-run 报告，避免旧 run、旧图、路径漂移或不可用 Blender 被误当成可增强输入。
...
| Phase 5 ENHANCE | 把 CAD 渲染图增强到照片级，并做一致性验收 | `*_enhanced.*`、`ENHANCEMENT_REPORT.json`、provider preset | 82% | provider 选择、增强闭环、同 run 一致性验收和确定性 `quality_summary` 已接入；剩余是更强语义/材质级多视角判断 |
| Phase 6 ANNOTATE / DELIVER | 标注、交付最终图片包和证据摘要 | `*_labeled_*.png`、`DELIVERY_PACKAGE.json`、可读摘要 | 60% | `photo3d-deliver` 已生成 active-run 最终包，并要求增强状态和 `quality_summary` 同时 accepted；后续仍需更强可视化验收和最终报告美化 |

**总体进展：约 81%。** 最大剩余缺口不在单个零件，而在 Phase 4 的更丰富逐视角可见实例证据，以及 Phase 5/6 的语义/材质级照片级判断：当前确定性质量门禁已经能基于 Blender 预检、路径/hash、画布、主体占比、亮度、对比度、饱和度和边缘密度阻断明显不可交付结果，但还不是 AI/人工语义审美裁决。
...
剩余：
- 增强结果的视觉质量验收还需要继续扩展，尤其是语义结构一致性、材质一致性和更接近人眼判断的照片级评分。
- 若要加入 `gpt-image-2-pro`，必须先做真实 backend adapter、配置隔离、一致性验收和安全测试，再进入白名单。

### Phase 6 ANNOTATE / DELIVER：60%
...
- 多视角照片级一致性仍需更严格的语义、材质和人工/AI 复核入口。
...
| 1 | 语义/材质级增强质量复核 | Phase 5 -> Phase 6 | 当前质量门禁是确定性指标，仍不能替代语义结构、材质真实感和人眼审美判断 | 引入可配置 AI/人工复核入口，但仍绑定 active run 和报告证据 |
| 2 | 常用模型库下一批 | Phase 2 | 继续提高不同产品零配置成图质量 | 更多常见外购件自动走可辨识 B 级或真实模型 |
