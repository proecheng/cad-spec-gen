...
| --- | --- |
| 更新日期 | 2026-05-05 |
| 当前主线 | `main` 已快进合并本轮 Phase 4 视觉回归门禁；待最终推送和清理 `codex/render-visual-regression` worktree/分支 |
| 管线 Phase 数 | 6 个：SPEC / CODEGEN / BUILD / RENDER / ENHANCE / ANNOTATE |
| 总体能力进展 | 约 79%（按 6 个 Phase 的工程化能力估算，不代表某个具体产品一次出图进度） |
| 当前主攻 Phase | Phase 4 RENDER：`render-visual-check` 契约层门禁本轮接入；下一步是 Phase 5 -> Phase 6 多视角照片级质量验收 |
| 最新功能基线 | `render-visual-check` Phase 4 渲染视觉/元件一致性门禁；上一轮为 `photo3d-deliver` 最终交付包 |
| 最新合并/进度提交 | 功能提交 `769873b feat(photo3d): 增加渲染视觉一致性门禁` 已快进合并到 `main`；同步修正提交 `9003bef chore(sync): 纳入渲染视觉门禁镜像` 已补齐被忽略的安装版工具镜像 |
| 最新归档计划提交 | `9ed3280 docs(project): 归档通用传动件计划` |
| 最近验证 | `python -m pytest tests\test_render_visual_regression.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` -> `160 passed`；Phase 4 周边回归 `28 passed` |
| 同步检查 | `python scripts\dev_sync.py --check` -> 通过；`git diff --check` -> 通过（仅 Windows 行尾提示） |
| 当前未跟踪 | 无运行时产物纳入；本轮新增工具、测试、计划文档和安装版镜像已提交。另有独立旧 worktree `.worktrees/generic-threaded-photo-autopilot`，本轮不触碰 |

## 一句话结论

cad-spec-gen 已形成 6 阶段 CAD 混合渲染管线。现在不是围绕某个零件做临时调参，而是在把“其他用户、其他产品也能傻瓜式出照片级 3D 图”的通用流程补齐：Phase 4 本轮新增 `render-visual-check`，能只按当前 active run 的契约证据检查视角、渲染文件、装配实例和 accepted baseline，防止新图少视角、少元件、旧 run 混用或路径/哈希漂移。
...
| Phase 6 ANNOTATE / DELIVER | 标注、交付最终图片包和证据摘要 | `*_labeled_*.png`、`DELIVERY_PACKAGE.json`、可读摘要 | 58% | `photo3d-deliver` 已生成 active-run 最终包；后续仍需更强可视化验收和最终报告美化 |

**总体进展：约 79%。** 最大剩余缺口不在单个零件，而在 Phase 5/6 的照片级视觉验收和 Phase 4 的像素级质量证据：契约层已能阻断少视角、少元件、旧 run 和路径漂移，下一步要继续补多视角材质一致性、照片级质量评分和 Blender 预检。
...
| 1 | 多视角照片级质量验收 | Phase 5 -> Phase 6 | 防止单视角看似可用但多视角材质、结构或语义不一致 | `ENHANCEMENT_REPORT.json` / `DELIVERY_PACKAGE.json` 中有更强质量证据 |
| 2 | Blender 环境预检和截图级回归 | Phase 4 | 契约层能防少件，仍需要证明 Blender 环境、画面和材质质量稳定 | 渲染前能发现 Blender/材质/灯光问题，渲染后有像素或截图级质量证据 |
...
- `photo3d-deliver` 默认只有增强验收 `accepted` 才复制最终增强图、源图和可唯一识别的标注图，并标记 `final_deliverable=true`；`preview` / `blocked` 只写证据报告，除非显式 `--include-preview` 才复制预览图，且 `final_deliverable` 仍为 false。
...
1. Phase 5 -> Phase 6：继续扩展增强质量验收，覆盖多视角一致性、材质一致性和照片级质量评分。
2. Phase 4：补 Blender 环境预检、材质/灯光质量检查和截图/像素级回归，让契约层门禁之后还有画面质量证据。
