...
| --- | --- |
| 更新日期 | 2026-05-05 |
| 当前主线 | `codex/provider-health-check` worktree 正在执行本轮；合并后推送 `main` 并清理 worktree/分支 |
| 管线 Phase 数 | 6 个：SPEC / CODEGEN / BUILD / RENDER / ENHANCE / ANNOTATE |
| 总体能力进展 | 约 72%（按 6 个 Phase 的工程化能力估算，不代表某个具体产品一次出图进度） |
| 当前主攻 Phase | Phase 5 ENHANCE：Provider 配置健康检查已完成，下一步是增强执行 + enhance-check 闭环入口 |
| 最新功能基线 | Provider 配置健康检查（本轮完成）；上一轮为 `5a7b4d1 feat(project-guide): 增加 provider 选择向导` |
| 最新合并/进度提交 | 本轮待合并；完成后以 `feat(project-guide): 增加 provider 配置健康检查` 为准 |
| 最新归档计划提交 | `9ed3280 docs(project): 归档通用传动件计划` |
| 最近验证 | `python -m pytest tests\test_photo3d_provider_health.py tests\test_project_guide.py tests\test_photo3d_provider_presets.py tests\test_photo3d_user_flow.py tests\test_photo3d_packaging_sync.py tests\test_dev_sync_check.py tests\test_data_dir_sync.py -q` -> `164 passed` |
| 同步检查 | `python scripts/dev_sync.py --check` -> 通过；`git diff --check` -> 通过（仅 Windows 行尾提示） |
| 当前未跟踪 | 本轮新增 `docs/superpowers/plans/2026-05-05-provider-health-check.md`；另有独立旧 worktree `.worktrees/generic-threaded-photo-autopilot`，本轮不触碰 |

## 一句话结论

cad-spec-gen 已形成 6 阶段 CAD 混合渲染管线。现在不是围绕某个零件做临时调参，而是在把“其他用户、其他产品也能傻瓜式出照片级 3D 图”的通用流程补齐：Phase 1-4 的 CAD/渲染证据链已基本可用，Phase 5 已把增强 provider 选择和配置健康状态接进普通用户/大模型向导，Phase 6 还需要继续强化增强验收、最终交付包和标注验收。
...
| Phase 5 ENHANCE | 把 CAD 渲染图增强到照片级，并做一致性验收 | `*_enhanced.*`、`ENHANCEMENT_REPORT.json`、provider preset | 72% | provider 白名单、普通用户文案、handoff、enhance-check、UI wizard 和配置健康检查已接入；下一步是增强执行与验收闭环 |
| Phase 6 ANNOTATE / DELIVER | 标注、交付最终图片包和证据摘要 | `*_labeled_*.png`、交付报告、可读摘要 | 38% | 基础阶段存在，但“傻瓜式最终交付包”和可视化验收还不够完整 |

**总体进展：约 72%。** 最大剩余缺口不在单个零件，而在 Phase 5-6：让非编程用户和大模型通过稳定向导完成增强执行、增强验收和最终交付。
...
剩余：
- 增强执行 + `enhance-check` 闭环入口：选择 provider 后自然进入增强验收，而不是让用户手拼下一条命令。
...
| 1 | 增强执行 + enhance-check 闭环入口 | Phase 5 -> Phase 6 | 选择 provider 后要自然进入增强验收，而不是让用户手拼下一条命令 | 增强完成后自动给出 accepted/preview/blocked 和下一步 |
| 2 | 最终交付包 | Phase 6 | 照片级结果需要可交付，不只是生成一张图 | 一个目录里有增强图、标注图、证据报告和用户摘要 |
| 3 | Blender 视觉回归和元件一致性检查 | Phase 4 | 防止出现“新渲染比旧渲染少元件”的通用问题 | 渲染阶段能报告元件数量/身份/视角证据是否漂移 |
| 4 | 常用模型库下一批 | Phase 2 | 继续提高不同产品零配置成图质量 | 更多常见外购件自动走可辨识 B 级或真实模型 |
| 5 | 新用户项目入口再简化 | Phase 1 -> Phase 6 | 把全流程串成少提问、多确认的项目向导 | 用户只说产品和目标，系统按 Phase 给下一步 |
...
1. Phase 5 -> Phase 6：把增强执行、`enhance-check` 和交付状态串成闭环，让用户看到 accepted/preview/blocked 后自然进入下一步。
2. Phase 6：设计最终交付包，把增强图、标注图、源渲染、证据报告和用户摘要放在一个可审计目录。
3. Phase 4：补 Blender 视觉回归和元件一致性检查，通用防止“新图比旧图少元件”这类问题。
