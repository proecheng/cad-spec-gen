...
- **状态**：rev 2（5 角色对抗审查 — 17 BLOCKER + 32 MAJOR + 21 MINOR 全闭）
...
```
tools/photo3d_jury.py                ← cli 入口 + 顶层 try/except 兜底 + 报告组装（薄壳）
tools/jury/
    __init__.py
    config.py                        ← profile 解析 / active 选取 / 校验 / 内置估价表
    cost.py                          ← 预估 + budget 守门
    input_evidence_binding.py        ← Layer 0 + sha256/active_run freeze + .jury.lock
...
tests/jury/
    __init__.py
    conftest.py                      ← autouse kill switch + dummy fixture key
    fixtures/
...
6. **Layer 0 → Layer 1 → Layer 2 串行硬约束**：每层只在前层全 pass 时才跑；fail 立即 return 不跑后续层
7. **active_run_id + sha256 在 Layer 0 freeze**：全程不重读；写报告前再算一次若漂移 → status=blocked
...
10. **api_base_url 路径不补全**：jury 不抄 `gemini_gen.py:46` 的 `/v1` 强制补全；用户自填完整 base
...
**关于 `api_base_url` 路径**：spec 要求用户填**完整 base 含 `/v1`**（如 `https://api.openai.com/v1`），jury 拼 `/chat/completions` 不做任何路径补全。**这与 `gemini_gen.py:46`（强制补 `/v1`）行为不同**——jury 实现禁止抄 gemini_gen 的拼接策略，要在 CP-3 实施时显式注释这一点（防止 implementer subagent 抄旧代码引入回归）。
...
| `needs_review` | "{K}/{M} 视角自动验收失败（{error_kinds}）；建议重跑或换 profile。" | `photo3d-jury --profile-id <next_id>` 列出 next 可用 profile（参考 `--list-profiles` 输出） |
| `blocked` | "证据与 active run 不一致（{first_blocking_code}）；不能作为照片级判定输入。" | `photo3d-recover --subsystem <X>` + 检查 enhance-check 是否真 accepted |

`{N}/{M}` / `{error_kinds}` / `{first_blocking_code}` 由实现层填充。
...
**关键字段约束**（防止下游 enhance-review 直接 blocked）：

- `subsystem` 必须 == cli `--subsystem`（jury 直接抄）
- `run_id` 必须 == jury Layer 0 冻结的 `active_run_id`（不是写报告时再读 ARTIFACT_INDEX，是冻结值）
- `source_reports.render_manifest` 必须用 `Path.as_posix()` 形态；与 jury Layer 0 当时读到的一致
- `source_reports.{render_manifest,enhancement_report}_sha256` 必须 == jury Layer 0 冻结的 sha256（**重要**：不是写报告时再算一次；冻结防止 jury 跑 N 分钟期间被 race condition 改坏 ENHANCEMENT_REPORT 后导致 hash mismatch；若写报告前再次读发现 sha 漂移，jury status=`blocked` 整体重写主报告，不写本文件）
...
2. **Key 永不出现在错误信息**：
   ...
   - **redact 规则同步覆盖** `error.url` / `error.headers` / `error.read()`：仅保留 host + path 模板（不含 query / fragment / Authorization / Cookie / Set-Cookie），防中转商把 `?api_key=xxx` 放 query string 时 url 通过 `HTTPError.geturl()` 暴露
   - 顶层 cli 必有 try/except 兜底捕获 `Exception`，**禁止裸抛**（防 Python default `sys.excepthook` 打印 frame locals 含 `api_key` 字符串）
   - `llm_client.py` 内 api_key 读出后立即装入 dataclass / 闭包，**不作为 stack frame 显式 long-lived local 变量**；不作为函数 default 参数（会进 inspect 签名）
   - `http.client.HTTPConnection.debuglevel = 0` 在 `llm_client.py` 顶部 import 后显式设置（覆盖外部 logging level=DEBUG / 用户全局设的 debuglevel=1，防 urllib 自动 dump 含 Authorization 的请求行到 stderr）
...
| `test_input_evidence_binding.py` (Layer 0 新文件) | 12 | 文件不存在 blocked / subsystem 不一致 / run_id 不一致 / `runs[active].active != True` blocked / `report.status != accepted` blocked / `delivery_status != accepted` blocked / `quality_summary.status != accepted` blocked / views=[] blocked / 视角数超 max_n_views blocked / 图大小超 max_image_bytes blocked / `--config` 越界无 `--allow-external-config` blocked / sha256 freeze 后再读漂移 blocked |
...
| `test_photo3d_jury.py` (cli 薄壳) | 10 | 缺 --subsystem 报错 / config 缺失 exit=2 / 超 budget 无 confirm exit=3 / 输入证据错 exit=1 / 全成功 exit=0 / Layer 1 fail → preview → exit=0 / LLM needs_review → exit=0 / `.jury.lock` 已存在 → exit=4 / `--list-profiles` 输出 profile 列表不调 LLM / `--dry-run` 跑 Layer 0/1 + 预估不写报告 / stderr 中文人话提示行存在（每 exit code 至少一行） |
...
3. **module-level `urlopen` patch**：单元测试默认 patch `tools.jury.llm_client.urlopen` 模块属性
...
6. **manual_smoke.py 排除**：`pyproject.toml` 加 `[tool.pytest.ini_options] addopts = "--ignore=tools/jury/manual_smoke.py"`（防 collect 阶段 import 触发真实 urlopen 副作用）；`[tool.coverage.run] omit = ["tools/jury/manual_smoke.py"]`（不计入 cov 分子分母）
...
- **CP-0 pre-flight + 目录脚手架**：
  - grep 验证假设（`tools/contract_io.py:write_json_atomic` 签名 / `tools/path_policy.py:assert_within_project` 签名 / `tools/enhancement_semantic_review.py:REQUIRED_SEMANTIC_CHECKS` tuple / `tools/enhance_consistency.py:MIN_PHOTO_CONTRAST_STDDEV` 值 / ENHANCEMENT_REPORT.json `views[]` 实际 keys / `gemini_gen.py:46` `/v1` 拼接行为对比 / `runs[active_run_id].active==True` 字段存在）
  - 新建 `tools/jury/__init__.py` / `tests/jury/__init__.py` / `tests/jury/conftest.py`（autouse kill switch）
  - 静态 fixture：`tests/jury/fixtures/sample_enhancement_report.json` + `sample_render_manifest.json` + `sample_artifact_index.json`
