...
**抉择**：§13 标题 = "v2.37.x 版本承诺与不变量（user-facing）"；3 条声明：

1. `schema_version=1` 在整个 v2.37.x patch 系列（v2.37.0 / v2.37.1 / v2.37.2 / v2.37.3 / v2.37.4 / 未来 v2.37.x）字段集不变
2. `CAD_JURY_DISABLE_LLM=1` 是 v2.37.x 唯一 kill switch 路径（不增新 env）
3. v2.37.1 时代生成的 sidecar / verdict 存档反序列化到 v2.37.4 零迁移（5-key 自动 `.get('matches_spec', True)` 兜底，v2.37.2 不变量 #11 锁定）
...
- **AC-3** §13 grep strict 关键短语（plan task 0 + Task 1 验证）：
  ...
  - `grep -c "零迁移\|不变量 #11" docs/cad-jury-config.md` ≥ 1（新 §13）
...
4. `grep -n "^## " docs/cad-jury-config.md` — 实测既有章节编号，确认 §11 + 附录 A 位置（spec D4 插入点验证）
