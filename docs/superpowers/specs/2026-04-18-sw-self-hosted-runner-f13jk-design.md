...
> **本 spec 在 runbook §7 F.2 段 line 187-189 follow-up 列表对应**：F-1.3j（workspace cleanup）+ F-1.3k（K1 第二数据点）；不新增 decision 号，是 #39 运维 follow-up（U：机械设计师跨 spec 时间轴可读性诉求；v4 L5-1 更正：F.2 spec 章节止于 §6+附录，无 §10）
...
```yaml
      - name: Pre-checkout workspace cleanup (F-1.3j 防 protected-ACL 残留)
        shell: pwsh
        env:
          PYTHONUTF8: "1"  # v4 L5 P2#19：与既有 step 一致，保证 pwsh Write-Host 中文 log 可读
        run: |
...
| **silent-fail 可观测性**（L：3 角色合并 + v5 P2#7 测试覆盖空洞承认）| best-effort 但**不静默** — 失败时通过 `::warning::` GitHub workflow command 在 Actions UI 顶部显示 annotation，避免长期失败被掩盖；连续失败可在 F-1.3l 触发条件里加"≥ 2 次连续 cleanup_residual annotation 升级 issue"。**测试覆盖空洞**：admin S1 已彻底清完 + cleanup step 兜底再删后，**生产路径下 `Test-Path -LiteralPath $cache` 不会再返 True，annotation 路径永不触发**；本 spec 接受此覆盖空洞，依赖未来 protected-ACL 复发时自然触发；如需自动测试钩子，可在 cleanup 内 inject `if ($env:F13J_FORCE_RESIDUAL) { New-Item -ItemType Directory -Force $cache | Out-Null }` 强制残留 fixture（v5 标注，不在本 spec 实施，留 F-1.3l follow-up） |
...
2. F.2 块**之后**追加新块（v2 模板，v3/v4 已增订）
3. **runbook §1 前置要求加 gh CLI ≥ 2.20**（v4 L5 P2#16）—— spec §3.1 P1.0 + §6 S5/S6 都用了 gh CLI 命令（`gh api runners` / `gh workflow run` / `gh run list --event` / `gh run watch --exit-status` / `gh run download`），其中 `--event` flag 自 gh CLI 2.20 起稳定，须在 §1 前置列表加一行：`- gh CLI ≥ 2.20（验证：gh --version；F-1.3j+k 的 runner Idle 检查 + workflow_dispatch 触发 + artifact 下载链路依赖此版本）`
4. **runbook §8.3 末尾加自动化补充行**（继承 v3 L3 P2#1 双向引用要求）
...
- AC-2.5 materials 业务承诺: ✅ `layers.materials.data.sldmat_files = <数值>` (> 0)（v4 L5 P2#10：与 §7 baseline `sw-inspect-deep.json.layers.materials.data.sldmat_files` 全路径风格统一）
- AC-3 K1 第二数据点：`layers.dispatch.data.elapsed_ms = <数值> ms` / `layers.dispatch.data.attached_existing_session = false`（v5 P2#3：与 §7 baseline 全路径风格统一）
...
承诺侧：
- [ ] 链路层 — runner 自启动 OK，workflow_dispatch 触发 pickup（push main 路径未在本 spec 复验，依据共享 runner / 同 workflow 论据继承 F.2 PASS pickup-only 结论；**已核 sw-smoke.yml `if: head_commit.message` 在 dispatch 时 `head_commit=null` 故 `contains()` 返 false、step 等价进入**——line 14-18 注释亦实证此 fallback 行为，dispatch 与 push 路径在 step 触发条件层等价；v4 L5 P2#26 措辞精确化）
...
|---|---|---|---|
| S1 | admin PowerShell 跑 §3.1（含 v3 P1.0 runner Idle 检查 + P1.1 LastWriteTime 复核（v5 P3#9 容差 ≤ ±48h，超出则停 ghrunner Task 排查）+ P1.2/3/4 一次性 fix） | 不 commit（手工动作） | 否 |
| **S2.0（v4 新增 L5-2 TDD 前置 / v5 L6-2 规格补完）** | 为 §3.4 改动**先写失败测试**（CLAUDE.md TDD 铁律）。**必须 1 commit**（RED 先于 GREEN 提交，0 commit 违反铁律）：<br>**(a) P4-E RED**：新增 `tests/test_assert_sw_inspect_schema.py::TestJsonDecodeError::test_empty_file_returns_65`，用 `tmp_path` pytest fixture 创建空文件 → `subprocess.run([sys.executable, "tools/assert_sw_inspect_schema.py", str(tmp_path / "empty.json")])` → 期望 retcode == 65 / stderr 含 "JSONDecodeError"。当前 main() 无 try/except → AssertionError 抛 retcode=1 → 测试 RED ✅<br>**(b) P4-F RED**：新增 `tests/test_sw_smoke_skip_guard.py::TestSkipGuard`，3 个 testcase fixture（1/2/3 个 testcase 的 junit xml，写入 `tmp_path`）→ 用 `subprocess.run([sys.executable, "-c", _inline_skip_guard_script(xml_path)])`（不是复制 heredoc，是把 sw-smoke.yml line 44-52 的 python 块抽到 fixture helper 函数 `_inline_skip_guard_script`）→ 期望 real==2 时 PASS / real==1 或 3 时 AssertionError。当前 sw-smoke.yml line 51 是 `>= 1` → 1 个 testcase 也 PASS → 测试 RED ✅ | **1 commit**（必须，TDD 铁律）：`test(sw-smoke): RED for retcode 65 + real==2 skip-guard` | 否 |
| **S2（v5 P2#4 拆 2 commit）** | **commit 1**：`fix(assert_schema): retcode 65 for JSONDecodeError`——改 `tools/assert_sw_inspect_schema.py` main() 加 try/except JSONDecodeError → retcode 65 + 友好 stderr，让 S2.0(a) RED 转 GREEN。<br>**commit 2**：`fix(sw-smoke): add cleanup + skip-guard tighten`——sw-smoke.yml 加 §3.2 cleanup step（steps[0]）+ §3.4 删 line 59 `\|\| true` + 改 line 51 skip-guard `real == 2`，让 S2.0(b) RED 转 GREEN | **2 commit** on feature branch | 否（feature branch push 不触发） |
| **S2.5（v3 修订 R + L4 P1#3 + v4 L5-4 简化）** | 本地 dry-run pwsh cleanup 脚本（3 fixture：workspace 不存在 / 含 .pytest_cache happy path / 不设 GITHUB_WORKSPACE），**仅查 happy-path 三分支**；**protected-ACL 路径靠 S4/S5 真 ghrunner 跑覆盖**（本地 procheng admin 身份不能复现 ghrunner 限权 EPERM）；~~可选第 4 fixture（runas + icacls）~~ **v4 删除该 fixture**——`runas /user:` 默认会弹密码 prompt 在脚本中卡住，如需高保真请直接走 S4 ghrunner 真跑 | 不 commit（手工预检） | 否 |
| **S3（v3 修订 L4 P2#7 + v4 L5 P2#17 宽松 + v5 P3#10 输出明示）** | 本地 `actionlint --version` ≥ 1.6.20（pwsh shell 支持自此版本起）后跑 `actionlint .github/workflows/sw-smoke.yml`；fallback 优先级：(a) 本机 actionlint → (b) `docker run --rm -v ${PWD}:/repo rhysd/actionlint:latest -color` → (c) **若 actionlint + docker 均无 → warn-only 跳过**（warn 输出位置：**console only**，本机 plan 阶段无 step summary；不阻塞 PR，记 follow-up F-1.3d 提前装）；预期：无 error / 允许 pwsh inline 命令的"unknown command" warning（已知噪声，可 ignore） | 不 commit（预检） | 否 |
| **S4（v3 修订 L4 P2#5 + v5 P2#8 失败定位命令）** | PR + merge to main → merge commit 自动触发 sw-smoke run-A（push event）。**run-A 用作"修复有效性烟雾"（不入 K1）**；如果 run-A fail，**一键查失败 step**：`gh run view <run-A-runId> --json jobs --jq '.jobs[].steps[] \| select(.conclusion=="failure") \| .name'` 或 `gh run view <runId> --log-failed`；判断分支：(a) 失败 step = `actions/checkout@v6` → S1 admin fix 未生效，**先复诊 S1**；(b) 失败 step = 其他 → 不阻塞 S5（concurrency 不传染失败，run-B 可独立 dispatch） | merge commit | 是（自动 push event） |
| **S5（v3 L4 P1#2/P2#4/P2#6 + v5 L6-1 物理可执行性澄清 + P3#11 首启信号）** | **前置 (i)**：等 S4 run-A 跑完释放 SW（持久授权下 `Get-Page SLDWORKS` 返回 null 后再等 ≥30s 余量）。**前置 (ii) ghrunner SW 首启**——**先验证 ghrunner desktop session active**：admin PowerShell 跑 `query session` 看是否有 `ghrunner ... Active` 行（runbook §4 Autologon 配置后开机自动进 desktop）；分两路：<br>**路 A（ghrunner 已 Active desktop session，推荐）**：在 ghrunner desktop（物理屏 / RDP / Remote Assistance）**人工启 SW 一次**——双击桌面 SW 图标 / 等 SW 启动完 / `File → Open` 任意 sldprt 让 sldmaterials 路径加载 / 关闭 SW；**完成信号**：admin 跑 `Test-Path "C:\Users\ghrunner\AppData\Roaming\SolidWorks\SolidWorks 2024\sldmaterials\*.sldmat"` 返 True（v5 P3#11 首启完成判据）。<br>**路 B（ghrunner 仅 service-style 拉起、无 desktop）**：`runas /user:CC-PC\ghrunner` 启 GUI SW **不可行**（Session 0 isolation）；这种情况下 v4 S2.5 删第 4 fixture 是同一原因（runas 弹密码 + 无 desktop 等价跑不通）。**fallback**：把 ghrunner Task Scheduler 临时改 `LogonType Interactive`，重启机器让 Autologon 生效，再走路 A。<br>**触发**：`gh workflow run sw-smoke.yml --ref main` 显式 workflow_dispatch；**取最新 dispatch run-id**：`gh run list --workflow sw-smoke.yml --event workflow_dispatch --limit 1 --json databaseId,url --jq '.[0]'` 拿到 run-B URL + runId | 不 commit | **是**（workflow_dispatch 显式触发，与 F.2 T3 一致） |
| **S6（v4 L5-3 取数链路闭环 + P2#24 退出码语义 + v5 P2#6 主路径明示）** | (a) 等跑完：`gh run watch <runId> --exit-status`（`--exit-status` 让 watch 退出码反映 conclusion，与 F.2 spec T4 一致）；(b) 下载 artifact：`gh run download <runId> --name sw-smoke-artifacts --dir .\f2-evidence-<runId>`；(c) 取数 — **主路径 pwsh ConvertFrom-Json**（与 S1/S2.5/S5 PowerShell 语言一致）：`Get-Content .\f2-evidence-<runId>\sw-inspect-deep.json | ConvertFrom-Json | ForEach-Object { $_.layers.dispatch.data \| Select-Object elapsed_ms, attached_existing_session; $_.layers.materials.data \| Select-Object sldmat_files }`；**cross-check 仅作可选**：bash + `jq -e '.layers.dispatch.data \| .attached_existing_session == false and .elapsed_ms >= 3000 and .elapsed_ms <= 15000'`（用于 OS 分隔符敏感时双重验证）；(d) 核 AC-1..AC-5（含 AC-2.5 / AC-3 attached 守卫 + 区间判读 / AC-4 自动） | 不 commit | — |
| **S7（v3 L4 P3#10 + v5 P2#4 commit 粒度明示）** | 4 处 runbook 改动（§7 F.2 块开头状态升级行 / §7 F.2 之后新块 / §1 前置加 gh CLI ≥ 2.20 / §8.3 末尾加 Stop-Process 自动化补充行）**单文件 1 commit OK**（runbook 单文件多处改属同一 logical unit "F-1.3j+k 验证收尾"），不强制拆 commit；commit message 末尾加 `[skip smoke]`；**忘加 [skip smoke] 至多多跑一次 ~57s 无 fail 风险**（runbook 改动不动 sw-smoke.yml，re-trigger 走全 PASS 路径） | 1 commit on main，`[skip smoke]` 末尾 | 否（`[skip smoke]` 跳过无意义触发） |
...
| **S2.0 TDD 必须 1 commit**（v5 L6-2）| RED 必须先于 GREEN 提交（CLAUDE.md 强制）；0 commit / 与 S2 合 commit 都会让"先写失败测试"语义被覆盖；fixture 用 pytest tmp_path 不污染 repo |
| **S2 拆 2 commit**（v5 P2#4）| commit 1 改 assert_schema_py（让 S2.0(a) RED→GREEN）/ commit 2 改 sw-smoke.yml 三处（让 S2.0(b) RED→GREEN）；分开便于 revert 任一侧 + 1 commit 1 logical unit |
| **S2.5 dry-run 局限明示**（R + v3 L4 P1#3 + v4 L5-4）| cleanup step 含 3 happy-path 分支（workspace nonexistent / cache present / SW 进程），本地 procheng admin 跑全 success **不证明** ghrunner 限权下也 success；protected-ACL 路径靠真 ghrunner 跑（S4 run-A / S5 run-B）覆盖；~~v3 第 4 fixture（runas + icacls）~~ v4 删（runas 弹密码不可行，且与 S5 路 B 同因不可行） |
| **S3 三级 fallback warn-only console**（v5 P3#10）| 本机无 actionlint + 无 docker → console-only warning 跳过，不阻塞 plan/PR；记 F-1.3d 提前装 |
| **S6 主路径 pwsh ConvertFrom-Json，jq 仅 cross-check**（v5 P2#6）| pwsh 与 S1/S2.5/S5 PowerShell 语言一致，避免 task 双轨膨胀；jq 仅在 OS 路径分隔符可疑时双重验证 |
...
- **v4（本稿）**：layer 5 技术融合性 5 维度审查（REVISE / 4 P1 + 6 P2 + 3 P3 延后）共 10 项合入：
  - **L5 P1**：(1) 全文 "F.2 spec §10 backlog" 错引修正为 "runbook §7 F.2 段 line 187-189 follow-up 列表"（F.2 spec 章节实际止于 §6+附录，无 §10 — 这是 v1/v2/v3 一路自信错引，layer 5 抓出）/ (2) §6 加 S2.0 TDD 前置 step（CLAUDE.md 强制 RED→GREEN，§3.4 P4-E/P4-F 先写失败测试再实现）/ (3) §6 S6 补 K1 取数链路完整命令（gh run download + ConvertFrom-Json + jq + gh run watch --exit-status）/ (4) §6 S2.5 删除 v3 第 4 fixture（runas 弹密码 prompt 实际跑不通），高保真路径靠 S4 ghrunner 真跑覆盖
  - **L5 P2**：(5) §3.2 cleanup step 加 `env: { PYTHONUTF8: "1" }`（与既有 step 一致 + pwsh 中文 log 可读）/ (6) §5.2 模板 `materials.sldmat_files` 改全路径 `layers.materials.data.sldmat_files`（与 runbook §7 baseline 风格统一）/ (7) §5 加 runbook §1 前置 gh CLI ≥ 2.20 改动要求 / (8) §6 S3 actionlint ≥ 1.6.20 + 三级 fallback (本机/docker/warn-only) / (9) §6 S6 `gh run watch --exit-status` 与 F.2 T4 一致 / (10) §5.2 第一条 checkbox 措辞精确化（"无 push payload 依赖" → "if head_commit.message dispatch 时 head_commit=null contains 返 false 等价进入"）
  - **L5 P3 延后**：§3.3 push vs dispatch 实证锚点 / PowerShell `-EA` vs `-ErrorAction` 风格统一 / `-LiteralPath` 评估
- **v5（本稿）**：layer 6 执行可读性 + v4 回归审查（REVISE / 2 P1 + 6 P2 + 3 P3）共 11 项全合：
  - **L6 P1**：(1) §6 S5 ghrunner runas SW UI 物理可执行性澄清（路 A: ghrunner desktop session active 时人工启 SW；路 B: service-style 不可行，临时改 LogonType Interactive）/ (2) §6 S2.0 TDD fixture 规格补完（pytest tmp_path / `tests/test_assert_sw_inspect_schema.py` / `tests/test_sw_smoke_skip_guard.py` / subprocess.run 而非 heredoc 复制 / 必须 1 commit 不可 0）
  - **L6 P2**：(3) §5.2 模板 `dispatch.elapsed_ms` 全路径化 / (4) §6 S2 拆 2 commit + S7 明示单文件 1 commit / (5) §6 决策表补 S2.0/S3/S6 行 / (6) §6 S6 主路径明示 pwsh 主 jq cross-check / (7) §3.2 P2-L silent-fail 覆盖空洞承认 / (8) §6 S4 加 `gh run view --log-failed` 一键失败定位
  - **L6 P3**：(9) §6 S1 LastWriteTime ±48h 容差 / (10) §6 S3 warn-only console only / (11) §6 S5 首启完成信号 sldmaterials/*.sldmat
  - **关键 catch**：L6-1 ghrunner runas SW UI 物理可执行性是真潜在 plan 阶段 BLOCKER（v4 S2.5 删第 4 fixture 同因 runas 弹密码，但 S5 同样 runas 不弹的机制 v4 没解释）；L6-2 S2.0 fixture 规格缺失会让 plan 阶段 subagent 直接拆不动 task
