...
def probe_toolbox_index_cache(cache_root: Path, toolbox_dir: Path) -> ProbeResult:
    """data 字段含 entry_count 和 by_standard: dict[str, int]（GB/DIN/ISO/ANSI 分组）。"""

def probe_material_files(info: SwInfo) -> ProbeResult:
    """快速扫：只数 sldmat / textures / p2m 文件数；不解析 XML。"""

def probe_warmup_artifacts(cache_root: Path) -> ProbeResult:
    """SA-2：lock 状态用 non-blocking try-acquire + immediate release；
    不与 sw-warmup 争夺锁。SAR-3：step_files 统计降级——若 scandir 出 > 5000
    文件，只报 count 跳过 size 计算。"""
...
- **toolbox_index**: cache_path / exists / entry_count / fingerprint_current / fingerprint_cached / stale / size_bytes / **by_standard**（ME-1：`dict[str, int]`，至少含 `GB` / `DIN` / `ISO` / `ANSI` 4 键，其他标准号归入 `other`；成本毫秒级）
- **materials**: sldmat_files / textures_categories / textures_total / p2m_files（**仅数量，不解析 XML；**本轮 YAGNI——物理属性解析见 §12 ME-2 升级路径）
