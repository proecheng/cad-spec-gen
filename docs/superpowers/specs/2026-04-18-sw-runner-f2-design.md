...
**审查修订记录**：5 角色（系统分析师/架构师/3D 设计师/软件测试员/机械设计师）一致 NO，P1×5 + P2×4 + P3×4 已全部接受并合入本稿（v2）。
...
| ID | 动作 | 命令要点 | 成功判据 | 超时 |
|---|---|---|---|---|
| T1 | 查 Task 触发 + daemon 进程 | `Get-ScheduledTaskInfo` + `Get-Process -Name Runner.Listener` | `LastRunTime > reboot_time` **且** `LastTaskResult = 267009`（任务正在跑）**且** Runner.Listener 进程存在、StartTime > reboot_time | 即时 |
| T2 | 等 runner 上线（与 baseline 对比） | `gh api ...runners` 轮询 + 与 PC7 baseline 比较 | `status=online, busy=false` **且** `runner.id` 同一个但 GitHub 端检测到状态变化（避免 stale heartbeat 假阳）| 15 s × 12 = 3 min（架构师建议放宽 1 min） |
...
| PC3 | Task 条目存在 | `Get-ScheduledTask -TaskName 'GitHub Actions Runner (sw-smoke)'` | 返回 1 项，State=Ready | 重跑 `setup-runner-task.ps1`（admin） |
| PC4 | Autologon 目标 = ghrunner | `reg query 'HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon' /v DefaultUserName` | DefaultUserName=ghrunner | 重跑 Sysinternals `Autologon.exe` 配 ghrunner |
...
| PC7 | **记 baseline（关键，给 T1/T2 用）** | 同时记下 `(reboot_time, runner_status, runner_started_at)` | 写入临时变量 | — |

PC7 baseline 例：
```
reboot_time   = 2026-04-XX HH:MM:SS（即将重启的时间）
runner_status = offline（当前确认）
...
| **C2** Task 未触发 / daemon 没起 | `LastRunTime ≤ reboot_time`（仍 1999 或时间未更新）/ `LastTaskResult = 267011`（从未跑）/ Runner.Listener 进程不存在 | (a) PowerShell `Start-ScheduledTask -TaskName 'GitHub Actions Runner (sw-smoke)'` 手动拉起; (b) 等 10 s 再查 | T1 再查；仍同失败状态 = FAIL |
| **C3** runner offline | T2 超时 3 min 仍 `status=offline` | 查 `D:\actions-runner\_diag\*.log` 末段 → 按 runbook §8.1 一次（如重启 run.cmd / 检查网络）| T2 再轮询；仍 offline = FAIL |
...
- runner_version：`<actions-runner-win-x64-X.XXX.X>`（D:\actions-runner\.runner 内查）
- 链路检查点：
...
| 回归测试（隐式）| sw-smoke 自身 | T4 conclusion=success → PASS clean；conclusion=failure 且 pickup OK → PASS pickup-only |
...
# PC7 — baseline 记录（关键）
$rebootTime = Get-Date  # 准备重启时间，给 T1/T2 比较用
...
Write-Host "C2 判据：LastRunTime > $rebootTimeStr ? $($taskInfo.LastRunTime -gt $rebootTime)"
...
if ($listener) {
  ...
  Write-Host "C2 判据：StartTime > $rebootTimeStr ? $($listener.StartTime -gt $rebootTime)"
} else {
...
# runner_version（从 .runner 文件读）
Get-Content D:\actions-runner\.runner | ConvertFrom-Json | Select-Object agentVersion
```
