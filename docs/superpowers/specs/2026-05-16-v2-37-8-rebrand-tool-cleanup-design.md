...
#### 3.1.0 archive sentinel marker（**rev 4 RISK-CRITICAL fix**）

**真实风险数据**：cad-spec-gen repo root 经 scout 实证含 **78 个 JSON 顶层有 `subsystem` 字段**（58 string + 20 dict）；包括 `cad/end_effector/.cad-spec-gen/CUSTOM_PARTS_AUDIT.json = "end_effector"` 等 production metadata。如果用户误传 `--archive-dir D:/Work/cad-spec-gen` + `--apply --from end_effector --to GISBOT`，将破坏 7+ production 文件。

**正金闸门制**：

- archive_dir 根目录**必须含 `.test-archive-marker` 文件**才接受作合法 archive_dir
- 不含 marker → exit=2 + stderr `[ERROR] archive_dir missing .test-archive-marker file (refuse to operate on non-archive directory)`
- marker 文件内容不约束（可空 / 可放说明文字）；仅文件存在性作 sentinel
- **本 PR 顺手 touch marker 到** `D:/Work/cad-tests/GISBOT/.test-archive-marker`（仓库外手动加，retro 文档记录命令）

**误传防御覆盖**：

| 误传路径 | sentinel 防护 |
| --- | --- |
| `D:/Work/cad-spec-gen`（repo root）| 无 marker → exit=2 ✓ |
| `D:/Work/cad-spec-gen/cad/end_effector` | 无 marker → exit=2 ✓ |
| `D:/Work/cad-spec-gen/.worktrees/...` | 无 marker → exit=2 ✓ |
| `D:/Work/cad-tests/GISBOT/`（合法）| marker 存在 → 继续 ✓ |

**rationale**：deny list 是黑名单（必须穷举所有不该扫的）；sentinel 是白名单（仅显式标识的可扫）。后者契约更严，新增 dev 工具 reuse 时也安全。
...
扫描 <archive_dir>:
  校验 (input validation, exit=2 early):
    ...
    not (archive_dir / ".test-archive-marker").is_file() → exit=2 "[ERROR] archive_dir missing .test-archive-marker file (refuse to operate on non-archive directory)"  (rev 4 RISK-CRITICAL — §3.1.0)
...
| `<archive_dir>/.test-archive-marker` 不存在（rev 4 RISK-CRITICAL）| stderr `[ERROR] archive_dir missing .test-archive-marker file ...` | **2** |
...
11. **archive sentinel marker（rev 4 RISK-CRITICAL）**：archive_dir 根目录必须含 `.test-archive-marker` 文件才接受；不含则 exit=2 拒绝操作（防误传 production repo 路径破坏 metadata）
...
| T15 | **archive sentinel marker 缺失 exit=2**（rev 4 RISK-CRITICAL）| tempdir 不放 `.test-archive-marker` / 跑 / exit=2 + stderr `archive_dir missing .test-archive-marker file` |
| T16 | archive sentinel marker 存在则继续（白盒）| tempdir 放空 `.test-archive-marker` + 1 JSON / 跑 `--apply` / 正常改写 |
...
| AC | 期望 | 验证 |
| --- | --- | --- |
| AC-1 | `python tools/dev/rebrand_test_archive.py --help` exit=0 + 输出含 Examples block + legend + Safety marker 说明 | shell |
| AC-2 | 16/16 单元测试 PASS（T1-T16，含 T15+T16 sentinel marker）| `pytest tests/dev/test_rebrand_test_archive.py -v` |
...
**做**：
- §11-N1 dev 工具（scan-all + dual-schema-aware + cross-platform-safe + archive sentinel marker）+ **16 单元测试** + 1 GISBOT 集成测
...
| **R10** | **误传 cad-spec-gen repo / production cad 目录破坏 metadata**（rev 4 RISK-CRITICAL）| archive sentinel marker `.test-archive-marker` 文件存在性校验作正金闸门；不含则 exit=2 早返；§3.1.0 + 不变量 #11 + T15/T16 测试硬保 |
...
用户 review 抓 1 项 RISK-CRITICAL（"改完比之前糟"路径）：

- **风险**：cad-spec-gen repo root 经 scout 实证 **78 JSON 顶层有 subsystem 字段**；7+ 是 production metadata（如 `cad/end_effector/.cad-spec-gen/CUSTOM_PARTS_AUDIT.json = "end_effector"`）；当前 deny list 不挡 `cad/` `.worktrees/` `tmp/`
- **攻击路径**：`python tools/dev/rebrand_test_archive.py . --from end_effector --to GISBOT --apply` 在 cad-spec-gen repo root 跑 → 7+ production 文件被破坏
- **fix**：archive sentinel marker — archive_dir 根目录必须含 `.test-archive-marker` 文件才接受；正金闸门替代黑名单维护负担（§3.1.0 + 不变量 #11 + T15/T16）
- **覆盖**：误传 cad-spec-gen repo root / cad/end_effector / .worktrees 任何路径 → 缺 marker → exit=2 拒绝
