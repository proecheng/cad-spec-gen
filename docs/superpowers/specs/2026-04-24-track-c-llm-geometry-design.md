...
| C-7 | IP-Adapter 对 FAL 的实现 | `fal-ai/flux-pro/v1.1-ultra` 的 `image_prompt` 参数传 enhanced_V1 base64 |
| C-8 | 向后兼容保证 | L0 路径代码不变；所有新路径在独立函数/条件分支中；现有测试不修改 |
| C-9 | L3 占位件标识 | STEP 文件头注释 `# ENRICHED_PLACEHOLDER — geometry approximated`，供下游识别 |
...
def _llm_extract_params(
    ...
    required_keys: list,   # ["od", "id", "thickness", "bolt_pcd"]
) -> dict | None:
    """从 spec 文本提取模板所需参数。失败返回 None，不抛异常。"""
```

**Prompt 约束**：
- 要求输出严格的 JSON，不含代码、不含解释文字
- 例：`{"od": 90, "id": 45, "thickness": 20, "bolt_pcd": 65, "bolt_count": 6}`
- 超时 10s，JSON 解析失败 → 返回 `None`；若 `route()` 结果为 `HIT_*` 则降级进入 L2；若 L2 亦被禁用则走 L3

**验收**：`ee_001_08`（法兰件）调用后 `factory()` 拿到完整参数，生成含螺栓孔的 CadQuery 几何，面数 ≥ 30。
...
for attempt in range(3):
    try:
        ...
        wp = ns["make_part"]()
        wp.val().exportStep(str(tmp_path))
        return code  # 成功
...
`fal_enhancer.py` 的 `enhance_image()` 新增可选参数 `ip_ref_path: str | None = None`：

```python
if ip_ref_path:
    payload["image_prompt"] = _encode_image_b64(ip_ref_path)
    payload["image_prompt_strength"] = cfg.get("ip_strength", 0.3)
```

`image_prompt_strength` 默认 0.3（轻度约束，不压制几何细节）。

### 4.4 ComfyUI 实现

`comfyui_enhancer.py` 在 workflow JSON 中激活已有 `IPAdapterNode`（现有 template 已包含，但 `image` 输入为空）：

```python
if ip_ref_path:
    workflow["IPAdapterNode"]["inputs"]["image"] = _load_image_base64(ip_ref_path)
    workflow["IPAdapterNode"]["inputs"]["weight"] = cfg.get("ip_strength", 0.3)
```
...
"enhance": {
  ...
  "ip_strength": 0.3,         // IP-Adapter 参考强度，0.0–1.0
  "enable_multiview_ref": true
}
```

`engineering` 后端（纯 PIL）不受影响，`shared_seed` 忽略。
...
```python
# 测试：给定法兰描述文字，LLM 能否提取正确参数
def test_l1_param_extraction_flange():
    spec_text = "法兰外径 90mm，中心孔 45mm，厚度 20mm，螺栓孔中心距 65mm，6 孔均布"
    params = _llm_extract_params("法兰盘", spec_text, "flange", ["od","id","thickness","bolt_pcd"])
    assert params is not None
    assert abs(params["od"] - 90) < 1
    assert params["bolt_count"] == 6
```

### 6.2 L2 测试

```python
# 测试：LLM 生成的代码是否能通过自我修正并导出有效 STEP
def test_l2_codegen_spring_mechanism():
    result = _llm_generate_cadquery("弹簧限力机构", spec_text, envelope=(50,50,80))
    assert result is not None  # 3次内成功
    assert Path(result).suffix == ".step"
...
```python
def test_multiview_shared_seed_produces_consistent_output(tmp_path):
    # 对同一 PNG 以相同 seed 增强两次，结果应完全相同
    r1 = engineering_enhancer.enhance_image(png, ..., seed=42)
    r2 = engineering_enhancer.enhance_image(png, ..., seed=42)
    assert _psnr(r1, r2) > 40  # 近似相同
```
