...
│  ├─ test_move_decision_to_history                          ← M-7 写入端守护（3 reason 枚举）
...
├─ class TestDecisionsEnvelopeIO                             ← M-7 IO 边界守护（**需加 1 测试**）
...
| 改动 | 覆盖率影响 |
|---|---|
| M-7 写入端删运行时校验（main line 419-423 物理 5 行 = 2 statement：1 `if` + 1 `raise`） | 分母 -2 statement |
| M-7 读取端加 IO 校验（约 4 statement：`for` + `if` + `raise` + comprehension/get 链） | 分母 +4 statement，分子 +4（新测试覆盖） |
| M-6 移 import | 不变（模块级 import 不计 statement 覆盖） |
| M-8 加 assert | 分母 +1，分子 +1（M-8 测试覆盖该 assert）→ 覆盖率几乎不变 |

**净影响**：分母 +3（2 写入端净 + 1 assert），分子 +5（IO 校验 4 + assert 1）；
覆盖率轻微 ↑，无 gate 风险。具体 statement 数 plan 实施时 `pytest --cov` 实测确认。

### §4.6 测试统计预估

- 删除：1 个测试方法 `test_move_decision_rejects_unknown_reason`（§4.2 锁定）
- 新增：2 个测试
  - `test_invalid_reason_assertion_holds_under_validate_contract`（M-8 assertion 契约）
  - `test_load_rejects_unknown_invalidation_reason_in_history`（M-7 IO 边界守护）
- 修改：0（refactor 行为一致）

**净增 = +1 个用例**（-1 + 2），CI 跑时间无感影响。
...
按"单一主题、可回滚 + TDD 严格"切 9 个 commit。**两个原则**：
1. **TDD 严格**：每个新 feature 先 RED 测试 commit → 后 GREEN 实现 commit；refactor
   类（M-7 写入端删校验、M-6 import 移位）TDD 不适用，单独 commit + 引用既有测试守护
2. **spec §11 closure mark（commit 8）排在所有代码改动 commit 之后**——避免回滚某个代码
   commit 后 spec 仍声明 closed

```
1. test(sw_config_broker): RED — M-8 assertion 契约测试（先 fail）
2. feat(sw_config_broker): M-8 — caller assert (commit 1 测试 GREEN)
3. test(sw_config_broker): RED — IO 边界守护测试（_load 拒绝未知 invalidation_reason）
4. feat(sw_config_broker): M-7 IO 边界 — _load_decisions_envelope 加读取端校验
   (commit 3 测试 GREEN)
5. refactor(sw_config_broker): M-7 — Literal type + 写入端删 _move 校验 + 删旧 ValueError 测试
6. chore(sw_config_broker): M-6 — 函数级 import 提到模块级
7. ci(mypy): mypy strict gate (sw_config_broker.py only)
8. chore(sw_config_lists_cache): I-4 加 1 行已知限制注释
9. docs(spec): §11 标 M-3/M-6/M-7/M-8/I-4 closed + 引用本 spec
```

每个 commit 主题清晰、可独立 review：
- commit 1-2：M-8 caller assert，TDD RED→GREEN
- commit 3-4：IO 边界（H1=A 决策的 new feature），TDD RED→GREEN
- commit 5：M-7 写入端 refactor（删旧 _move 校验 + 改 Literal 类型 + 删旧测试），
  reviewer 焦点 = "写入端可以删 OK，因为 mypy 编译期 + 读取端 runtime 双重守护"
- commit 6-9：M-6 / mypy CI / I-4 注释 / spec §11 closure mark（最后）
...
- [ ] 9 个 commit 按 §6.2 顺序提交（含 2 对 RED→GREEN：M-8 + IO 边界）
- [ ] 既有 broker 测试全 PASS（基线 ~199，前置条件 PR #21 merge 后）
- [ ] M-8 + IO 边界守护共 2 个新测试 PASS
