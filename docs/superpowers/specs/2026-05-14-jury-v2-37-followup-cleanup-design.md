...
- **规模**：小 单 PR；总 diff ≈ 50 行（2 行实现 + 测试 + 文档 + docstring 对齐；**CI gate 既存，本 PR 不加只验证**）
- **状态**：brainstorming 完成 + **5 层审查通过 + plan 阶段 scout grep 抓到 1 处 spec 漂移**（self / cynical re-read / code-spec 对照 / edge-case hunter 7 findings 全修 / 5 角色并行 + holistic dry-run 共 34 findings 分类处置 / writing-plans 入口 scout 发现 dev_sync --check 已在 tests.yml 既存）；待用户复审 → writing-plans
...
| `.github/workflows/tests.yml` | **无变动**——scout grep 发现 dev_sync `--check` step 在 `test` + `regression` 两 job 已存在（line 50-58 + line 105-115）；本 PR 仅在 spec / 不变量 / plan task 验证此既存 gate 工作正常（不变量 §6 #8 改为「锁定既存 gate」语义）|
...
8. **CI gate 锁定既存 dev_sync 同步**：scout grep 证实 `.github/workflows/tests.yml` `test` 与 `regression` 两 job 已含 `python scripts/dev_sync.py --check` step（line 50-58 + line 105-115，v2.10 起既存）；本 PR **不加只锁**——把这条既存 gate 升级为 spec 永久不变量（删除等于破坏本不变量，本 PR 之后任何 PR 不得删 step）；下次任何 PR 改 canonical 漏 sync 仍被 CI 拦（edge-case finding #2 误判已存 gate 为缺失，layer 6 scout 抓到）
...
（原计划的 `chore(ci): tests.yml 加 dev_sync gate` commit 被 layer 6 scout 移除——既存 gate 不需要再加；本 PR 通过不变量 §6 #8 升级锁定既存 gate 语义。）
...
9. **验证 `.github/workflows/tests.yml` 既存 dev_sync gate** — 已 scout 确认 `test` job line 50-58 + `regression` job line 105-115 含 `python scripts/dev_sync.py --check`；plan 重新 grep 复核（防 base sha 飘移），写进 plan task 1 验证步而非「加 step」（不变量 §6 #8 已升级为「锁既存 gate」）
