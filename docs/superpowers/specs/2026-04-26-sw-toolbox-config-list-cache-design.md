...
gen_std_parts.main(spec)
 └─ resolver.prewarm(bom)                                         ← 新 API
     └─ for adapter in self.adapters:
         try: adapter.prewarm(bom_queries)                        ← 新 abstract method
         except Exception as e: log.warning(...)                  # prewarm 失败不阻 codegen

# sw_toolbox_adapter.prewarm 实现：
#   ├─ for q in bom: sldprt = self.find_sldprt(q, spec)         ← 已有 API（catalog-only，不调 COM）
#   │   if sldprt: collect.append(sldprt.path)
...
#   ├─ if envelope_invalidated(cache): cache = _empty_cache()    # SW 版本 / Toolbox 路径变
#   ├─ miss = [sldprt for sldprt in collect if not _entry_valid(cache, sldprt)]
#   ├─ if miss:
#   │   ├─ subprocess.run([worker, '--batch'], input=json.dumps(miss), timeout=180)
#   │   ├─ for entry in stdout_json: cache['entries'][path] = {mtime, size, configs}
#   │   └─ _save_config_lists_cache(cache)                      # atomic .tmp + os.replace
#   └─ # cache 全命中 / batch 失败 → 不动 cache，prewarm 静默 return

# BOM loop 内现有 broker.resolve_config_for_part 不变；
# _list_configs_via_com 改为：
#   ├─ in-process L2 cache 命中 → return                        # 现有
#   ├─ 持久化 cache 命中 → 填 L2 → return                       ← 新增
#   └─ fallback：单件 spawn worker（罕见兜底）                   # 现有
```
...
| `broker.prewarm_config_lists(sldprt_list)` | 新 public API | diff cache → spawn batch worker → 写回 | `adapters/solidworks/sw_config_broker.py` |
| `PartsAdapter.prewarm(bom)` | 新 abstract method | 基类 default no-op；sw_toolbox 实现 | `adapters/parts/base.py` + `sw_toolbox_adapter.py` |
| `PartsResolver.prewarm(bom)` | 新 method | 派发给所有 adapter（per-adapter try/except） | `parts_resolver.py` |
...
```python
def prewarm_config_lists(sldprt_list):
    cache = _load_config_lists_cache()
    miss = _diff_miss(cache, sldprt_list)
    if not miss:
        return
    try:
...
不存在该问题：新文件不存在时 `_empty_cache()` 视为空 → 走原行为（fallback 单件 spawn）。
...
### 7.3 PartsAdapter base class 加 abstract method

- 基类 `prewarm(bom)` 给 default no-op 实现 → 现有 adapter（jinja_primitive / step_pool / partcad / bd_warehouse）无需改动
- sw_toolbox_adapter 重写实现实际逻辑
