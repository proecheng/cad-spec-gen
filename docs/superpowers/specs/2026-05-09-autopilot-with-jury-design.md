...
| 1.1 | 2026-05-09 | spec 自审 grep verify 修事实错：测试文件名 `test_cad_pipeline_autopilot.py` → `test_photo3d_autopilot.py`（实测）|
...
6. 测试 `tests/test_photo3d_autopilot.py` 加 5+ 集成用例 + 新建 `tests/test_photo3d_autopilot_jury_detection.py` 含 9 helper 单测
...
tests/test_photo3d_autopilot.py   ← 加 5+ 集成用例 + 回归守门（~80 行 add-only）
...
追加 `tests/test_photo3d_autopilot.py`（~80 行）：
