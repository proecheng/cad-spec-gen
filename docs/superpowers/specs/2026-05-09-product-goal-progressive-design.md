...
- **break-change**：v2.25.0 现有 needs_kpi_confirmation 路径 exit code 非 0 → 0（外行用户不感知；CI 脚本可能受影响）
...
4. 改 `tools/project_guide.py:write_project_goal_guide`：缺 KPI 时**先写 state file**；ordinary_user_message 改为引导加 --resume；返 exit=0（不再 exit≠0）
...
- v2.25.0 一次说全行为完全保持（缺 KPI 时**唯一变化**：exit code 1→0 + 多写一个 state file）
...
def command_return_code_for_project_guide(report: dict[str, Any]) -> int:
    ...
    # NEW: needs_kpi_confirmation 返 0（之前可能是 1）
    if status in {
        "ready_for_cad_spec",
        "needs_kpi_confirmation",  # NEW: 异步多轮模式正常状态
        "needs_subsystem_confirmation",
        "not_yet_implemented",
    }:
        return 0
    # 其他错误状态保持
    if status in {"unknown_subsystem", "needs_design_doc", "needs_product_goal"}:
        return 0  # 信息提示类，非错误
    return 1  # 解析失败 / 内部错误
```
...
v2.25.0 needs_kpi_confirmation 路径 exit code 由非 0 → 0：
- 在 spec / PROGRESS / README 显式标记
- CI / 脚本如果依赖 exit≠0 判断"用户输入不全"需迁移到读 status 字段
- 提供迁移建议：`if [ "$(jq -r .status PROJECT_GUIDE.json)" = "needs_kpi_confirmation" ]; then ... fi`
...
### 7.3 break-change（明确标记）

**唯一 break**：v2.25.0 `needs_kpi_confirmation` 路径 exit code 由非 0 → 0。

CI / 脚本如果依赖 exit code 判断"输入不全"需迁移：
```bash
# v2.25.0 老脚本（exit code 检测）：
if cad-spec-gen --product-goal "$GOAL"; then
    echo "OK"
else
    echo "input incomplete"
fi

# v2.31.0 新脚本（status 字段检测）：
...
- `docs/PROGRESS.md` 加 v2.31.0 入口（含 break-change 标记）
- `README.md` 用法示例增多轮渐进
- 本 spec commit
...
| 风险 | 影响 | 缓解 |
|---|---|---|
| break-change exit code 影响 CI 脚本 | 用户脚本可能误判 | spec / PROGRESS / README 显式标记 + 迁移示例 |
...
3. **break-change 守门**：v2.25.0 老用例如有依赖 needs_kpi_confirmation exit≠0 的明确改成期望 0；spec 标记
