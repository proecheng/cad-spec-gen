...
| T22 | `test_invalidate_save_any_exception_warns_and_continues` | mock save 抛 RuntimeError / KeyError / TypeError 任一 → 全部 catch + log.warning（含"envelope save 失败"关键词）+ worker spawn 仍调用 + prewarm 不抛（fire-and-forget 契约）|
...
按调用次数 / 模式分支返预设结果，**统计调用次数**：

```python
locking_calls = []
def fake_locking(fd, mode, nbytes):
    locking_calls.append(("LK_NBLCK" if mode == msvcrt.LK_NBLCK else "LK_UNLCK", fd, nbytes))
    if mode == msvcrt.LK_NBLCK and len(locking_calls) <= contention_count:
        raise OSError("contended")
    return None
monkeypatch.setattr(real_msvcrt, "locking", fake_locking)
```

#### `_save_config_lists_cache` 调用顺序追踪
...
def tracking_save(cache):
    ...
    # 真写盘以便磁盘验证测试（T28/T29）也能复用此 mock
...
I-3 测试在 Linux CI 上跑：
- D6（T14/T15）测试**真实** `sys.platform != "win32"` 路径，无需 mock
- D1-D5/D7/D8（T1-T13/T16-T18）测试 Windows 行为，必须 monkeypatch `sys.platform = "win32"` + `monkeypatch.setitem(sys.modules, "msvcrt", fake_msvcrt_module)` 以让 `import msvcrt` 命中 fake 模块

参考 memory `feedback_ci_windows_only_skipif.md` 既有 Pattern B（`requires_windows` marker）— 本 PR 30+ 测试规模适用 Pattern B：在 conftest.py 加 `pytestmark = pytest.mark.requires_windows` 给整个 test file，让 Linux CI skip 大部分测试（仅 T14/T15 在 Linux 上 active）。
