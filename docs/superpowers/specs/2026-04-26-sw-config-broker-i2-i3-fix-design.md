...
def prewarm_config_lists(sldprt_list: list[str]) -> None:
    ...
    if cache_mod._envelope_invalidated(cache):
        ...
        try:
            ...
            # save 失败不阻断 prewarm（与 line 612 现有 try/except 同模式）
            log.warning(
...
| 异常源 | 处理 | 是否冒到 caller |
|--------|------|----------------|
| **新增的 envelope 升级 `_save_config_lists_cache`** | catch OSError + log.warning + 继续走 worker spawn | ❌ 内部消化 |
| 新增 save 抛非 OSError | 不 catch → 自然上抛 | ✅ 异常类型严格性 |
| `detect_solidworks()` 异常 | 不 catch（detect 内部自有兜底返 SwInfo(installed=False)）| ❌ 既有行为不变 |
| 既有 line 612 的 `_save_config_lists_cache` | 既有 try/except 已包 | ❌ 既有行为不变 |

### §5.3 设计 invariant

1. **prewarm 永远不阻断 BOM loop**：任何 prewarm 内部错误（save 失败 / worker 失败 / detect 失败）都 log.warning 静默；BOM loop 通过 reader 单件 fallback 路径仍能拿到 configs → 不影响图像质量
...
| I-2 不静默 fail invalid envelope | save 升级失败是配置问题信号，必须 log.warning 让用户能 grep 到 — 但不 hard fail（fire-and-forget 语义）|
...
| # | 测试名 | 验证目标 |
|---|--------|----------|
| T7 | `test_lock_never_raises_timeout_at_60s` | 撞锁 60s+ → 不抛 OSError；最终拿到锁仍正常 yield |
...
| # | 测试名 | 验证目标 |
|---|--------|----------|
| T16 | `test_lock_banner_contains_all_required_keywords` | banner stderr 包含全部 4 关键词：`检测到另一个 codegen` / `Ctrl+C` / `不要手动删除` / `图像与 BOM 不一致` |
...
| T20 | `test_invalidate_save_content_correct` | save 调用时 cache dict 字面值：schema_version=1 / sw_version=新 / toolbox_path=新 / entries={} 严格匹配；`generated_at` 仅验存在 + 符合 ISO 8601 时间字符串格式（dynamic 值不字面匹配）|
...
| T22 | `test_invalidate_save_non_oserror_propagates` | mock save 抛 RuntimeError → 上抛 + worker spawn 不调用 |
...
| T26 | `test_invalidate_save_propagates_detect_unexpected_exception` | detect_solidworks 抛 RuntimeError → 上抛（既有行为不变；防"为 I-2 修复加宽 except"）|
...
- **新增 30 测试均不依赖 cadquery / partcad / bd_warehouse**（纯 `unittest.mock` + 标准库 + msvcrt mock）
- 修复后**本地**：1251 + 30 = **1281 passed**；其他数字（35 failed / 32 skipped / 13 errors / 2 collection errors）**0 增长**（即不破坏既有失败状态）
...
.venv/Scripts/python.exe -m pytest -q \
  ...
  2>&1 | tail -3
# 期望: 1281 passed, ... 35 failed, 32 skipped, ... 13 errors
...
|--------|------|--------------------------|-------|
| Commit 1（I-2 先做）| `prewarm_config_lists` 加 try/except + save | T19-T30（§6.2 I-2 矩阵）| 12 |
...
> **mapping 注意**：测试编号 T1-T30 按 §6.1 (I-3) → §6.2 (I-2) 章节顺序编排；但 commit 顺序 I-2 在前 I-3 在后，所以 commit 1 跑 T19-T30，commit 2 跑 T1-T18。**plan 阶段拆 task 时务必按 commit-test mapping 落地，不按编号自然顺序**。
...
## 测试
- 新增 30 测试（I-3: 18 / I-2: 12）按 14 个行为维度详尽覆盖
...
- [ ] 改动 surgical（仅 1 文件 + 2 函数 + 30 测试 + 1 import + 3 常量）
