...
| T13 | `test_lock_path_parent_auto_mkdir` | lock_path.parent 不存在 → mkdir(parents=True, exist_ok=True) 自动建 |
...
| T20 | `test_invalidate_save_content_correct` | save 调用时 cache dict 含：schema_version=1 / sw_version=新 / toolbox_path=新 / entries={} 严格匹配 |
...
| # | 测试名 | 验证目标 |
|---|--------|----------|
| T28 | `test_invalidate_save_disk_json_schema_full_match` | save 后磁盘 JSON 反序列化 → 5 字段全员（schema_version / generated_at / sw_version / toolbox_path / entries）+ entries == {} 严格相等 |
...
- 既有 1251（PR #19 merge 后 baseline）
- 新增 30 测试（T1-T30）
- 总 **1281 passed**

CI 跑全平台 7/7 SUCCESS 后即可 merge。

---

## §7 PR 结构与 commit 划分

### §7.1 commit 拆分（2 commits）

**Commit 1（I-2 先做）**：`fix(sw_config_broker): I-2 envelope 升级立即落盘（PR #19 review fix）`
- 改 `prewarm_config_lists` invalidate 分支末尾加 try/except + save
- 加 12 测试（T19-T30）
- 通过 → push CI → 全绿

**Commit 2（I-3 后做）**：`fix(sw_config_broker): I-3 锁等待永不超时 + 进度提示（PR #19 review fix）`
- 重写 `_project_file_lock`
- 加 18 测试（T1-T18）
- 加常量 + 文案 + import time
- 通过 → push CI → 全绿
