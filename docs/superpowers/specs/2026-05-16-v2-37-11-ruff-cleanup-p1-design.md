...
> **Spec rev**：rev 1（首版；按 CLAUDE.md §6 spec <100 行默认轻档 review = self-review + Task 0 scout grep）
...
清掉 ruff 421 errors 里 **safe `--fix` 自动可修一类 = F401 + F541 + F811 + E401 共 154 条**；ruff `--fix` 跳过的 case（估 5-15 条）逐条加 `# noqa: <code>  # <5-15 字中文>` 注释；P1 完结时 `ruff check --select=F401,F541,F811,E401 .` exit 0。

| 改动 | 严重度 | 内容 | 工作量 |
| --- | --- | --- | --- |
| **改动 1** | LOW | `ruff check --fix --select=F401,F541,F811,E401 .` 一键产物（154 条自动 fix）| ~10min |
| **改动 2** | LOW | ruff `--fix` 跳过的 case 加 noqa + 5-15 字中文注释 | ~30-60min |
...
预期 fix 154 条；其中：
- F401 unused-import 128 条 — 删除 `import x` 或 `from x import y` 形式
- F541 f-string-missing-placeholders 18 条 — `f"foo"` → `"foo"`
- F811 redefined-while-unused 7 条 — 删除前一个未用的 def/import
- E401 multiple-imports-on-one-line 1 条 — `import a, b` → `import a` + `import b`

**改动 2 — ruff `--fix` 跳过的 case 加 noqa**

跑改动 1 后再跑 `ruff check --select=F401,F541,F811,E401 .` 收集剩余条数 N（估 0-15）。逐条评估归类，加 `# noqa: <code>  # <5-15 字中文>` 注释。
...
- **AC-1 ruff 4 类清零**：`ruff check --select=F401,F541,F811,E401 .` exit 0 / `--statistics` 输出 F401/F541/F811/E401 行计数为 0
...
| Task | 行为 | 验收 |
|---|---|---|
| **0** | Task 0 scout grep — 跑 `ruff check --select=F401,F541,F811,E401 . --statistics` 实跑数字校准 == spec 声明 154；列出涉及的 file paths（防 spec 自漂移）| 实跑 statistics == spec / file list 留 plan 文档 |
| **1** | `ruff check --fix --select=F401,F541,F811,E401 .` 跑 → diff 检视 | `git diff --stat` 看变动文件数 / 行数 |
| **2** | 跑全套件 `pytest` 复跑 | 3244 PASS / 0 regression |
| **3** | `git add -A` + commit-impl-1 `chore(ruff): apply --fix F401+F541+F811+E401` | commit 落地 |
| **4** | 再跑 `ruff check --select=F401,F541,F811,E401 .` 收集 fix 跳过的剩余条目（按 file:line 列出） | 剩余条数 N（估 0-15）|
...
**总 commit 数 = 5**：spec rev 1 + plan + impl-1 + impl-2 + retro
...
| 类型 | 路径 |
| --- | --- |
| spec | `docs/superpowers/specs/2026-05-16-v2-37-11-ruff-cleanup-p1-design.md`（本文件，rev 1）|
...
✅ self-review 通过；§7 文案行数偏差 inline fix 已落（"<100 行" → "234 行但按用户明确授权走轻档"）。
...
（首版 rev 1 尚无 §12 follow-up；PR 实施后若发现 spec 漏洞或 lesson 在 retro 中开 §12 表登记）

---

**rev 1 写讫**。下一步：用户 review 本 spec 文档（路径上方），通过后进 writing-plans 出 plan 文件。
