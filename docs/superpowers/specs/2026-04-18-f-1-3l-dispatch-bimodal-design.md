...
两阶段 + 分层持久化。**Phase 顺序严格不可并行**：Phase 1 merge main ▶ Phase 2 开工（见 §8.0 前置 checklist） ▶ Phase 3 验收。
...
│   ├── **状态控制**：每轮开始前等待 sldWorkerManager 服务 Running 且稳定 ≥ 3s
│   │   （奇数轮 Stop-Process → sleep 30s → Wait-ForProcess Running / 偶数轮 Wait-ForProcess Running）
│   │   避免 baseline 组 SW 状态抖动（见 §10 R10）
│   └── 交替实验：10 轮，奇数轮前 Stop-Process sldWorkerManager / 偶数轮 baseline
│   判定后：HIT → 跳 H2，直接跑 H3；FALSIFIED → 进 H2；INCONCLUSIVE → 补 +5 点重判
│
├── H2 COM ROT / registration cache（次；**被动观测型**）
│   │   **设计变更说明**（第 4 轮审查 E3 发现）：原"DEBUG_USE_COCREATEINSTANCEEX 开关证伪"
│   │   实验不可行 — `probe_dispatch` 每轮调 `ExitApp()`（sw_probe.py:608）让 SW 进程
│   │   退出，per-process ROT cache 在测量窗口外被销毁；假设在当前工具下原则上不可证伪。
│   │   改为**被动观测**：每轮 subprocess 额外读 ROT 状态作为相关性证据
│   ├── 临时探针：com_rot_entry_count / com_registration_timestamp
│   │   实现：`pythoncom.GetRunningObjectTable().Enum()` 读 ROT 条目数与 SldWorks 相关条目的时间戳
│   └── 观察实验：10 轮 subprocess 正常调 probe_dispatch，顺路记录 ROT 探针
│       （**不做**干预，与 baseline 并列意义消失）
│   判定后：
│     - Spearman(com_rot_entry_count, elapsed_ms) ≥ 0.7 → H2 HIT（相关性强，作为根因线索）
│     - 相关系数 < 0.4 → H2 FALSIFIED
│     - 0.4 ≤ < 0.7 → H2 INCONCLUSIVE（补 +5 点重判）
│     - HIT 仅作为 F-1.3l-Q2 新假设的切入点，不直接认定因果
│
...
| `tests/test_sw_probe_dispatch_per_step.py`（新建） | 覆盖冷启 / attach / timeout / 3 步异常 / t0 起点回归 / 最小值截断（E2：per_step_ms < 1ms 截断为 1 避免与"未到达"哨兵 0 冲突） |
...
**Phase 1 代码规范**（G4）：所有新增代码的 docstring / 行注释使用中文，遵循 `CLAUDE.md` "语言规范（强制）"段；标识符保持英文不变。
...
| `scripts/f13l_h1_license_test.ps1` | 新建·临时 | H1 交替实验脚本，10 轮 CSV 输出。**调用方式**：PS1 内部用 `python -c "from adapters.solidworks.sw_probe import probe_dispatch; ..."` **Python 直调**，不走 `cad_pipeline.py sw-inspect` CLI — CLI 无 `--layer` 参数无法单独跑 dispatch 层。每轮 stderr 落盘 `f2-evidence-f13l/h1_iter_{N}.stderr`（F1） |
| `scripts/f13l_h2_rot_observe.py` | 新建·临时 | H2 **被动观测**（第 4 轮 E3 改名）：10 轮 subprocess 调 probe_dispatch，每轮顺路读 ROT 状态；无干预无交替；输出含 com_rot_* 字段的 CSV |
| `scripts/f13l_h3_gradient_scan.ps1` | 新建·临时 | H3 间隔梯度 5/10/15/20/30/45/60min × 3 次 = 21 点；同样 Python 直调 |
| `scripts/f13l_analyze.py` | 新建·临时 | **一键分析**：`--csv X.csv --hypothesis H1/H2/H3` 读 CSV → pandas groupby / Spearman 相关 → 输出三态决策 + `describe()` 全表 + 样本量（便于人工复核，F2）。支持 `--merge-all` 合并 H1/H2/H3 三份 CSV 供 §8 (3) AC-3 区间计算（D3） |
| `adapters/solidworks/sw_probe.py` | 临时 patch | `probe_investigation_env()` 辅助函数，返回临时探针字段（license_session_age_sec / com_rot_entry_count / com_registration_timestamp / defender_last_scan_age_sec / defender_rtp_state）；不进 schema。**注**：E3 改方向后 `DEBUG_USE_COCREATEINSTANCEEX` 开关**不再需要**，从 patch 中移除。所有临时代码加统一注释 `# TEMP(F-1.3l): 删除请见 F13L_REVERT_CHECKLIST.md 第 X 项`（G3） |
| `f2-evidence-f13l/` | 新建·gitignored | CSV + 每轮 stderr 归档（本地磁盘）。**Phase 2 完毕前**统一 commit 到 `archive/f13l-evidence` 分支（不进 main），防本地磁盘丢数据（G2） |
| `F13L_REVERT_CHECKLIST.md` | 新建·临时 | 调查开始时写，收尾 revert 时逐项勾（防漏删）。含 workflow 文件行号 + 每条临时代码的 file:line 引用 |
...
**统一 CSV schema**（所有 H1/H2/H3 脚本都输出完全相同的 15 列；不适用字段填空 → pandas 读为 NaN）：
...
PS1 实验脚本（e.g. f13l_h1_license_test.ps1）
  ...
  │    # E1: 每轮开始前等待 sldWorkerManager Running 且稳定 ≥ 3s
  │    Wait-ForService sldWorkerManager -MinStableSeconds 3
  │    # Python 单行直调 probe_dispatch，stdout 打印 JSON 行，stderr 落盘（F1）
  │    $json = python -c "
  │      import json
  ...
  │    " 2> "f2-evidence-f13l/h1_iter_$iter.stderr"
  │    追加 CSV 一行（15 列统一 schema；不适用列留空；哨兵 -1 序列化为空 → pandas NaN）
  └─> f2-evidence-f13l/h1_license.csv

一键分析（scripts/f13l_analyze.py）
  统一预处理：
    df = pandas.read_csv(na_values=['', '-1'])
          .pipe(lambda df: df[df['attached_existing_session'] != True])  # 过滤 attach 行
          .dropna(subset=['dispatch_ms'])                                # 过滤"抛异常"行
  H1（kill vs baseline）/ H3（interval bins）分析：
    df.groupby(flag).describe()
    两组 per_step_ms 中位数差 ≥ 2x → 输出 "HIT"
    两组差 < 1.5x → 输出 "FALSIFIED"
    1.5x ≤ 差 < 2x → 输出 "INCONCLUSIVE, need +5 samples"
  H2（被动观测型）分析：
    scipy.stats.spearmanr(df['com_rot_entry_count'], df['elapsed_ms'])
    相关系数 ≥ 0.7 → "HIT"（作为 F-1.3l-Q2 线索，非因果）
    相关系数 < 0.4 → "FALSIFIED"
    0.4 ≤ < 0.7 → "INCONCLUSIVE"
  合并分析（`--merge-all`，D3）：
    三份 CSV 横向合并（按 ts_utc 排序）为单一 df，供 §8 (3) AC-3 区间
    计算（浅档中位数 / 深档中位数）使用
  输出（F2）：除单行决策外，stdout 打印完整 describe 表 + 样本量，
  写入 `f2-evidence-f13l/analysis_{H}.log` 便于事后复核
...
- **perf_counter 精度截断**（E2）：`int(elapsed_sec * 1000)` 在 <1ms 步会舍入为 0，与"未到达"哨兵 0 冲突。probe_dispatch 实现应对**任何成功跑过的步**取 `max(int(elapsed_sec * 1000), 1)` 最小截断为 1ms；0 专门保留给"未到达"语义
- **JSON 字段内部**保留真实值 -1 / 0 / ≥1；**CSV 序列化**时，-1 → 空字段（便于 pandas 读为 NaN，避免 `mean()` 把 -1 当合法值计算）；0 保持为 "0"（表示"有意义的零"，attach 路径 or 未到达）；≥1 真实测量值
...
- **Phase 2 实验脚本单轮失败不中断 loop**：跑 10 轮某次 COM 抖动只丢一行 CSV，不全废；stderr 落盘 `f2-evidence-f13l/h{X}_iter_{N}.stderr`（F1）便于事后追溯
...
- H2 **被动观测型**无副作用：只读 ROT，不改任何进程/服务状态（E3 改型后 DEBUG 开关已移除，无需回滚）
...
- CSV 输出前最小 validation：
  ...
  * 例：H1 脚本输出的行必须有 `kill_flag / dispatch_ms ... / license_session_age_sec` 非 NaN；
       `interval_min / com_rot_* / defender_*` 留空为合法
  * H2（被动观测型）：必须有 `dispatch_ms ... / com_rot_entry_count / com_registration_timestamp` 非 NaN；
       `kill_flag / interval_min / license_session_age_sec / defender_*` 留空为合法
...
### 8.0 Phase 2 开工前置 checklist（D2）

Phase 2 开工前必须全部勾选：

- [ ] Phase 1 PR 已 merge 到 main（§8 (1) 所有条件满足）
- [ ] runner 当前 idle（`gh run list --workflow=sw-smoke.yml --status=in_progress` 返回空）
- [ ] 建好 `F13L_REVERT_CHECKLIST.md` 并初始化行号映射
- [ ] workflow `runs-on` 追加 `f13l-exclusive` label 的 PR 已开（草稿态，Phase 2 结束 revert）
- [ ] 告知团队 F-1.3l 调查窗口，期间 sw-smoke CI 会被 `f13l-exclusive` 挡住
...
2. **Phase 2 至少 1 条假设有明确结论**
   - **HIT** 判据（按假设类型区分）：
     * H1 / H3（分组对比型）：`scripts/f13l_analyze.py` 两组中位数差 ≥ 2x
     * H2（被动观测型，E3 改型后）：Spearman(com_rot_entry_count, elapsed_ms) ≥ 0.7，
       仅作为 F-1.3l-Q2 的根因线索，不直接认定因果
   - **INCONCLUSIVE 处理路径**：
     * H1 / H3：差 1.5-2x，自动补采 +5 点最多 2 轮（共 10 点）；仍 INCONCLUSIVE 视为 FALSIFIED
     * H2：相关系数 0.4-0.7，自动补采 +5 点最多 2 轮；仍 INCONCLUSIVE 视为 FALSIFIED
     * 根据"用户零操作"原则，`scripts/f13l_run.ps1 --auto-chain` 内部自动执行此补采流程
   - **全伪**（H1 FALSIFIED + H2 相关性 < 0.4 + H3 FALSIFIED）：
     * 触发 F-1.3l-Q2 升级假设
...
4. **runbook §7 F-1.3l 根因记录段写入**
   ...
   - 数据链接（`f2-evidence-f13l/` CSV 的 git sha + 行数 + `archive/f13l-evidence` 分支 commit sha，G2）
   - **复现模板**（G1，3-5 行）：
     ```
     复现步骤（约 N 分钟）：
     1. git checkout <phase-2-commit-sha-from-checklist>  # 含 F-1.3l 脚本的 snapshot
     2. .\scripts\f13l_run.ps1 --hypothesis H<X>
     3. diff f2-evidence-f13l/h<X>_*.csv <baseline-csv-sha>  # 与当时数据对比
...
6. **临时探针 git revert**
   ...
   - [ ] `scripts/f13l_h2_rot_observe.py`
   ...
   - [ ] 确认 `archive/f13l-evidence` 分支已 push 到 origin（G2，防本地磁盘丢数据）
   - [ ] grep `# TEMP(F-1.3l):` 全库扫描，确认所有临时注释已清理（G3）
   - **保留不 revert**：`per_step_ms` 字段、schema 迁移、AC-2/AC-3 断言、Phase 1 单测 — 这些是 F-1.3l 留下的永久基础设施
...
- ❌ 假设 4+（SW 启动抖动 / Windows Update / Hyper-V 抢占 / **Windows Power Plan P-state**（E4））— 仅当 3 条全伪才升级到 F-1.3l-Q2（不在当前 spec 范围）
...
- ❌ 给临时探针写单测（ROI 低）
- ❌ 修 `probe_dispatch` 让其支持"单进程内多轮 dispatch"测 per-process cache（E3 讨论后确认；会与 attach 路径 elapsed_ms=0 冲突，工具扩改 ROI 低）
...
| R7 | H2 假设在当前 probe_dispatch 实现下不可证伪（per-process ROT cache 在 ExitApp 后即消失） | 中 | H2 变相"注定 FALSIFIED"，浪费实验预算 | 第 4 轮审查 E3 已把 H2 改为**被动观测型**（读 ROT 状态做 Spearman 相关性），不再是证伪实验；HIT 作为 F-1.3l-Q2 线索，不直接认定因果 |
...
| R10 | H1 baseline 组 SW 状态本身不稳定（sldWorkerManager 刚自启未完全 ready） | 中 | baseline 组 timing 噪声大，组间对比无效 | §3 H1 加"每轮开始前 Wait-ForService sldWorkerManager 稳定 ≥ 3s"（E1） |
| R11 | per_step_ms 快步（<1ms）舍入为 0 与"未到达"哨兵冲突 | 低 | 分析时误判"visible / exitapp 未到达"，数据污染 | probe_dispatch 对成功跑过的步取 `max(int(sec*1000), 1)`；`test_sw_probe_dispatch_per_step.py` 加最小值截断测试（E2） |
| R12 | Phase 2 脚本静默失败（e.g. python import 错，stderr 丢） | 中 | 半年后看 CSV 缺行不知道原因 | 每轮 stderr 落盘 `f2-evidence-f13l/h{X}_iter_{N}.stderr`（F1） |
| R13 | f2-evidence-f13l/ CSV 本地磁盘坏丢失 | 中 | F-1.3l 数据永久丢失，无法复现验证 | Phase 2 完毕前统一 commit 到 `archive/f13l-evidence` 分支推到 origin（G2） |
