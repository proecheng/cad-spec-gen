...
### 新增文件
```
src/cad_spec_gen/data/python_tools/enhance_budget.py       # M-1 跨 SP 通用 LoopBudget
...
│   └── fal_comfy_adapter.py    # 包装现有 fal_comfy_enhancer.py 保 ControlNet
└── rules/
...
**Files:**
- Create: `src/cad_spec_gen/data/tools/jury_loop/__init__.py`
- Create: `src/cad_spec_gen/data/tools/jury_loop/rules/.gitkeep`
...
```python
# src/cad_spec_gen/data/tools/jury_loop/__init__.py
...
def builtin_yaml_path() -> Path:
    ...
    return Path(str(files("cad_spec_gen.data.tools.jury_loop.rules") / "photoreal_v1.yaml"))
```
...
```bash
git add src/cad_spec_gen/data/tools/jury_loop/__init__.py \
        src/cad_spec_gen/data/tools/jury_loop/rules/.gitkeep \
        tests/jury_loop/__init__.py \
        tests/jury_loop/conftest.py
...
**Files:**
- Create: `src/cad_spec_gen/data/tools/jury_loop/secrets_scrubber.py`
...
from cad_spec_gen.data.tools.jury_loop.secrets_scrubber import scrub_secrets
...
```python
# src/cad_spec_gen/data/tools/jury_loop/secrets_scrubber.py
...
```bash
git add src/cad_spec_gen/data/tools/jury_loop/secrets_scrubber.py tests/jury_loop/test_secrets_scrubber.py
...
**Files:**
- Create: `src/cad_spec_gen/data/tools/jury_loop/reason_parser.py`
...
from cad_spec_gen.data.tools.jury_loop.reason_parser import (
    BUILTIN_TAGS,
    parse_reason,
    reason_sanitized,
)
...
```python
# src/cad_spec_gen/data/tools/jury_loop/reason_parser.py
...
```bash
git add src/cad_spec_gen/data/tools/jury_loop/reason_parser.py tests/jury_loop/test_reason_parser.py
...
**Files:**
- Create: `src/cad_spec_gen/data/tools/jury_loop/rules/photoreal_v1.yaml`

- [ ] **Step 1：写 yaml**

```yaml
# src/cad_spec_gen/data/tools/jury_loop/rules/photoreal_v1.yaml
...
Run: `python -c "import yaml; yaml.safe_load(open('src/cad_spec_gen/data/tools/jury_loop/rules/photoreal_v1.yaml','r',encoding='utf-8').read())"`
Expected: 不抛 yaml.YAMLError

- [ ] **Step 3：commit**

```bash
git add src/cad_spec_gen/data/tools/jury_loop/rules/photoreal_v1.yaml
...
**Files:**
- Create: `src/cad_spec_gen/data/tools/jury_loop/rule_table.py`
...
from cad_spec_gen.data.tools.jury_loop.rule_table import (
    KNOWN_PARAMS,
    RuleTableLoadError,
...
class TestLookup:
    def test_single_tag_hit(self) -> None:
        table = load_rule_table()
        result = lookup(table, tags={"plastic_look"}, backend="fal_comfy")
    ...
    def test_multi_tag_hit_addons_dedup_preserve_order(self) -> None:
        table = load_rule_table()
        result = lookup(table, tags={"plastic_look", "flat_light"}, backend="fal_comfy")
    ...
    def test_no_match_returns_empty(self) -> None:
        table = load_rule_table()
        result = lookup(table, tags={"unmapped_tag"}, backend="fal")
        assert result.prompt_addons == []
        assert result.param_overrides == {}

    def test_param_overrides_per_backend_isolated(self) -> None:
        table = load_rule_table()
        result_fal = lookup(table, tags={"plastic_look"}, backend="fal")
        result_fc = lookup(table, tags={"plastic_look"}, backend="fal_comfy")
        # fal 段不含 fal_comfy 字段，反之亦然
        assert "denoise_strength" not in result_fal.param_overrides
        assert "guidance_scale" not in result_fc.param_overrides


class TestParamRangeClamp:
    def test_clamp_strength_to_unit_range(self) -> None:
        # M-12 越界 clamp + warnings
        from cad_spec_gen.data.tools.jury_loop.rule_table import _clamp_param
...
```python
# src/cad_spec_gen/data/tools/jury_loop/rule_table.py
...
def load_rule_table(
    ...
    builtin_text = files("cad_spec_gen.data.tools.jury_loop.rules").joinpath(_BUILTIN_RESOURCE).read_text(encoding="utf-8")
    try:
...
```bash
git add src/cad_spec_gen/data/tools/jury_loop/rule_table.py \
        tests/jury_loop/test_rule_table.py \
        tests/jury_loop/fixtures/user_rule_yaml_valid.yaml \
        tests/jury_loop/fixtures/user_rule_yaml_unknown_field.yaml \
        tests/jury_loop/fixtures/user_rule_yaml_wrong_schema.yaml
...
**Files:**
- Create: `src/cad_spec_gen/data/tools/jury_loop/backends/__init__.py`
- Create: `src/cad_spec_gen/data/tools/jury_loop/backends/protocol.py`
...
- BACKEND_REGISTRY 启动时含 `gemini_chat_image` / `openai_images_edit` / `fal_comfy` 三个 key
...
**Files:**
- Create: `src/cad_spec_gen/data/tools/jury_loop/backends/gemini_chat_image.py`
...
**Files:**
- Create: `src/cad_spec_gen/data/tools/jury_loop/backends/openai_images_edit.py`
...
### Task 2.5.4：fal_comfy_adapter（包装现有 fal_comfy_enhancer.py）

**Files:**
- Create: `src/cad_spec_gen/data/tools/jury_loop/backends/fal_comfy_adapter.py`
- Test: `tests/jury_loop/test_backend_fal_comfy.py`

包装现有 `src/cad_spec_gen/data/python_tools/fal_comfy_enhancer.py:enhance_image()`。`known_params = {"denoise_strength": (0.0, 1.0), "cfg_scale": (1.0, 30.0), "canny_strength": (0.0, 1.0), "canny_end_pct": (0.0, 1.0), "depth_strength": (0.0, 1.0), "steps": (1, 200), "guidance_scale": (1.0, 30.0)}`；`supports_controlnet=True`；`estimate_cost_usd = 0.18`（实测后调）。

异常映射：现有 fal_comfy_enhancer 抛的 `fal_client.exceptions.*` 映射到 BackendError 子类。

测试：
- 正常调用走入 fal_comfy_enhancer.enhance_image() (mock)
- supports_controlnet 返 True
- known_params 含 ControlNet 参数 (canny_strength / depth_strength)

提交：`feat(jury-loop): fal_comfy_adapter 包装现有 enhancer + ControlNet 标记`
...
**Files:**
- Modify: `src/cad_spec_gen/data/tools/jury_loop/rule_table.py`
- Modify: `tests/jury_loop/test_rule_table.py`

回头改 rule_table.py：
1. `lookup(table, tags, backend_kind: str)` 签名改 str（去掉 `Literal["fal","fal_comfy"]`）
2. `_clamp_param(key, value, backend_kind)` 增加 backend_kind 参数；改为从 `BACKEND_REGISTRY[backend_kind].known_params` 拿白名单 + 范围而非读静态 KNOWN_PARAMS
3. 删除 rule_table.py 顶部静态 KNOWN_PARAMS 常量；保留为 backwards-compat re-export 但内部不用

测试加：
- lookup(backend_kind="gemini_chat_image") → temperature 字段命中 + clamp
- lookup(backend_kind="fal_comfy") → canny_strength 命中
...
**Files:**
- Create: `src/cad_spec_gen/data/tools/jury_loop/llm_fallback.py`
...
**Files:**
- Create: `src/cad_spec_gen/data/tools/jury_loop/score_select.py`
...
**Files:**
- Create: `src/cad_spec_gen/data/python_tools/enhance_budget.py`
...
**Files:**
- Create: `src/cad_spec_gen/data/tools/jury_loop/metadata.py`
...
**Files:**
- Create: `src/cad_spec_gen/data/tools/jury_loop/orchestrator.py`
...
try:
    ...
    metadata.write_degraded_sidecar(view, render_dir, error=e)
```
