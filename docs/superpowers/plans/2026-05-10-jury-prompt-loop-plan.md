...
**Architecture:** 9 个新模块 (`src/cad_spec_gen/data/tools/jury_loop/` 下) 全部纯函数 + 1 个跨 SP 通用 budget 模块；唯一副作用持有者是 `orchestrator.py`；`cmd_enhance` 内嵌视角级 hook + try/except 异常隔离 + LoopBudget 跨视角累计；photo3d-jury CLI 扩 `--single-view --image` 支持 single-image 评分；ENHANCEMENT_REPORT.json 条件性追加 `loop_summary` 顶层段。
...
├── rule_table.py           # 纯函数 + yaml safe_load：tags+backend → (addons, params)
...
按 spec §3 实现注记：`BACKEND_RETRY_COST_USD = {"fal": 0.20, "fal_comfy": 0.18}`；`JURY_LLM_CALL_COST_USD = 0.005`；`USD_TO_CNY_RATE = 7.2`；`LoopBudget(cap_usd, n_views)` 含 `_lock: threading.Lock`；`try_spend(amount) -> bool`、`record_actual(amount)`、`spent` 只读、`extra_cost_cny` 属性；`estimate_retry_cost(backend, with_jury: bool)` 函数返预估金额。
...
def run_loop_if_eligible(
    ...
    enhance_image_callable: Callable,   # cmd_enhance 注入，避免循环依赖
) -> LoopResult:
    """单视角闭环。LoopResult 含 final_path / loop_status / sidecar 数据。"""
```

实现 §3 所有 10 步 + 8 个 Gate；调用 score_select 策略；retry 失败按 §4.6 错误码细分（OPS-MAJOR-5）映射 retry_failed/rate_limited/quota/auth；写 sidecar 经 metadata 模块；返回 final_path 给 cmd_enhance 用作 hero_image 更新。

测试 (用 mock 注入 jury subprocess + enhance_image)：
- Gate-1 backend=gemini → loop_disabled
...
- Gate-8 retry HTTP 429 → retry_rate_limited
- Gate-8 retry HTTP 401 → retry_auth_failed
...
try:
    loop_result = orchestrator.run_loop_if_eligible(
        view=view, backend=backend, rc=rc, baseline_path=baseline_path,
        budget=budget, project_root=PROJECT_ROOT, config=jury_loop_config,
        enhance_image_callable=_enhance_fn,
    )
...
按 spec §4.1：顶层 `enabled=true` + `cost_cap_usd=1.5`；`advanced` 子段含 5 项；schema 校验函数检测顶层与 advanced 同名 key 共存 → ValueError（DRIFT-MAJOR-4）。

实现 `JuryLoopConfig` dataclass + `load_jury_loop_config(pipeline_config_dict) -> JuryLoopConfig`。

提交：`feat(jury-loop): pipeline_config.json jury_loop 段 + JuryLoopConfig（M-5 + M-6 + M-8）`
