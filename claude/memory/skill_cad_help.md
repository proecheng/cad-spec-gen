...
2. **多路径搜索** — 同一信息可能存在于多处（环境变量、配置文件、代码默认值），全部检查：
   ...
   - 渲染工具：`D:/cad-skill/tools/hybrid_render/` > `D:/cad-skill/cad/end_effector/` > `D:/cad-skill/tools/blender/`
   - prompt模板：`D:/cad-skill/tools/hybrid_render/prompts/` > `D:/cad-skill/cad/end_effector/prompt_*.txt`
...
| 组件 | 实际路径 |
|------|----------|
| gemini_gen.py | `D:/imageProduce/gemini_gen.py` |
| Gemini配置 | `~/.claude/gemini_image_config.json`（api_key + api_base_url + model） |
| Gemini API | `https://api.duckcoding.ai/v1`（中转代理，key格式 `sk-*`） |
| Gemini模型 | `gemini-3-pro-image-preview-4k` |
| 混合渲染工具 | `D:/cad-skill/tools/hybrid_render/`（check_env.py, prompt_builder.py） |
| Blender | `D:/cad-skill/tools/blender/blender.exe`（4.2.10 LTS） |
| 渲染配置引擎 | `D:/cad-skill/cad/<subsystem>/render_config.py`（示例：end_effector） |
| prompt模板 | `D:/cad-skill/tools/hybrid_render/prompts/`（若存在）或由 prompt_builder.py 生成 |
...
5. Blender: 搜索 D:/cad-skill/tools/blender/blender.exe → --version (需 4.x LTS)
...
7. Gemini AI增强 (按优先级逐项检查，任一通过即✅):
   ...
   c. 检查 gemini_gen.py 是否存在: D:/imageProduce/gemini_gen.py 或 $GEMINI_GEN_PATH
   d. 运行 D:/cad-skill/tools/hybrid_render/check_env.py (如存在)
...
环境检查结果:
  ...
  ✅ ezdxf 1.4.3
  ✅ matplotlib 3.10.8
  ✅ Blender 4.2.10 LTS (D:/cad-skill/tools/blender/blender.exe)
...
- Blender: 下载 Blender 4.2 LTS portable 到 `D:/cad-skill/tools/blender/`
- Gemini: 运行 `python D:/imageProduce/gemini_gen.py --config` 启动配置向导
...
```python
# 决策逻辑:
1. 扫描 D:/cad-skill/cad/*/render_config.json 找已配置的子系统
2. 扫描 D:/cad-skill/cad/*/build_all.py 找已实现的子系统
3. 扫描 D:/GISBOT/cad/output/*.step, *.dxf, *.glb, *.png, *.jpg 统计产物
...
Step 1: 创建目录和配置
  mkdir D:/cad-skill/cad/<subsystem_name>/
  复制模板: D:/cad-skill/docs/templates/render_config_template.json → D:/cad-skill/cad/<name>/render_config.json
...
情况A: 用户想直接渲染 → 运行命令
  ...
  cd D:/cad-skill/cad/<subsystem> && python build_all.py --render

  # 或单独渲染特定视角 (示例：end_effector)
  D:/cad-skill/tools/blender/blender.exe -b -P D:/cad-skill/cad/<subsystem>/render_3d.py -- \
    --config D:/cad-skill/cad/<subsystem>/render_config.json

  # 爆炸图
  D:/cad-skill/tools/blender/blender.exe -b -P D:/cad-skill/cad/<subsystem>/render_exploded.py -- \
    --config D:/cad-skill/cad/<subsystem>/render_config.json

情况B: 用户还没有GLB → 引导先构建
  1. python D:/cad-skill/cad/<subsystem>/build_all.py  → 生成 STEP + DXF
...
**搜索优先**：先读 `~/.claude/gemini_image_config.json` 获取实际配置，再回答。
...
实际配置 (~/.claude/gemini_image_config.json):
...
核心工具:
  gemini_gen.py:     D:/imageProduce/gemini_gen.py (全局命令行工具)
  prompt_builder.py: D:/cad-skill/tools/hybrid_render/prompt_builder.py (prompt生成)
  check_env.py:      D:/cad-skill/tools/hybrid_render/check_env.py (环境检查)

prompt模板 (搜索 D:/cad-skill/tools/hybrid_render/prompts/ 和 D:/cad-skill/cad/<subsystem>/):
  prompt_enhance.txt  — 标准视角通用增强 (材质/光照/环境)
  prompt_exploded.txt — 爆炸图专用 (保留装配线)
  prompt_ortho.txt    — 正交图专用 (保留工程特征)

核心原则:
  Gemini只"换皮"不改几何，prompt首行写 "Keep ALL geometry EXACTLY"

使用方式:
  python D:/imageProduce/gemini_gen.py "prompt文本" --image base.png
...
Q: Blender 找不到 / 启动失败
A: 确认 D:/cad-skill/tools/blender/blender.exe 存在，Blender 4.2 LTS portable版
...
Q: Gemini API 报错
A: 1. 检查 ~/.claude/gemini_image_config.json 是否存在且格式正确
   2. 确认 api_base_url 和 model 是否匹配你的服务商
   3. 检查网络连接 (中转代理可能需要科学上网)
   4. 运行 python D:/imageProduce/gemini_gen.py --config 重新配置
...
A: 检查 part_pattern 是否匹配实际零件名
   用 python -c "import glob; print(glob.glob('D:/GISBOT/cad/output/*.glb'))" 看实际文件名
...
├── tools/
│   ├── blender/blender.exe            ← Blender 4.2 LTS portable
│   ├── hybrid_render/                 ← 混合渲染工具
│   │   ├── check_env.py               ← 环境检查脚本
│   │   └── prompt_builder.py          ← Prompt模板生成
│   ├── bom_parser.py                  ← BOM解析器
│   ├── cad_spec_gen.py                ← CAD Spec生成器
│   └── cad_pipeline_agent_guide.md    ← 通用Agent集成指南
├── docs/templates/                    ← 模板
...
└── claude/memory/skill_cad_help.md    ← 本文件 (Claude Code版帮助)

参考实现: D:/cad-skill/cad/end_effector/ (§4末端执行器, 14脚本, 8 STEP + 11 DXF)

D:/GISBOT/cad/output/              ← 输出目录 (留在GISBOT项目中)
├── XX-000_assembly.step/.glb      ← 总装配
├── XX-NNN_*.step                  ← 子装配STEP
├── XX-NNN-NN_*.dxf                ← 2D工程图DXF
└── *.png / *.jpg                  ← 渲染结果

D:/imageProduce/gemini_gen.py      ← Gemini图生图全局工具 (项目外)
~/.claude/gemini_image_config.json ← Gemini API配置 (key/url/model)
```
...
```python
# 扫描逻辑:
1. glob D:/cad-skill/cad/*/render_config.json → 已配置子系统列表
2. glob D:/cad-skill/cad/*/build_all.py → 已实现子系统列表
3. glob D:/GISBOT/cad/output/*.step → STEP产物数
4. glob D:/GISBOT/cad/output/*.dxf → DXF产物数
5. glob D:/GISBOT/cad/output/*.glb → GLB产物数
...
第2层: 技能知识文档 (可直接作为 system prompt)
  D:/cad-skill/tools/cad_pipeline_agent_guide.md  ← 通用版 (无Claude Code依赖)
  D:/cad-skill/claude/memory/skill_cad_help.md     ← Claude Code版

第3层: 斜杠命令 (仅 Claude Code CLI)
  D:/cad-skill/.claude/commands/cad-help.md
  D:/cad-skill/.claude/commands/mechdesign.md

接入示例:

  GLM-4 + Function Calling:
    system_prompt = open("D:/cad-skill/tools/cad_pipeline_agent_guide.md").read()
    tools = [{"name": "run_shell", "description": "执行shell命令"}]
    → GLM读取指南 → 按流程调用 build_all.py / render_3d.py 等

  GPT-4 + Assistants API:
    上传 D:/cad-skill/tools/cad_pipeline_agent_guide.md 为知识文件
  ...
  任何 Agent 框架:
    1. 给 LLM 喂 D:/cad-skill/tools/cad_pipeline_agent_guide.md 作为知识
    2. 提供 shell/subprocess 执行能力
    3. LLM 按文档指引生成命令并执行

通用版导出:
  D:/cad-skill/tools/cad_pipeline_agent_guide.md
...
2. **运行解析器**:
   ```bash
   python D:/cad-skill/tools/bom_parser.py docs/design/NN-*设计.md          # 树形输出
   python D:/cad-skill/tools/bom_parser.py docs/design/NN-*设计.md --json   # JSON输出
   python D:/cad-skill/tools/bom_parser.py docs/design/NN-*设计.md --summary # 仅统计
   ```
...
4. **无BOM时**: 如果解析器报 "未找到 BOM 表"
   ...
   - 给出模板: `D:/cad-skill/docs/templates/bom_section_template.md`
   - 引导用户按模板补充
...
1. **生成 CAD_SPEC**: 对指定子系统运行提取器
   ```bash
   python D:/cad-skill/tools/cad_spec_gen.py docs/design/NN-*设计.md           # 单个子系统
   python D:/cad-skill/tools/cad_spec_gen.py docs/design/NN-*设计.md --force   # 强制重生成
   python D:/cad-skill/tools/cad_spec_gen.py --all                             # 全部18个子系统
   ```

2. **查看已有 CAD_SPEC**: 读取 `D:/cad-skill/cad/<subsystem>/CAD_SPEC.md`
...
4. **模板**: `D:/cad-skill/docs/templates/cad_spec_template.md`（空白模板带填写说明）
