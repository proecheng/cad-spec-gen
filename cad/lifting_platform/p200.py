...
│ Design doc ref : §3.1 左支撑条 — 40×260×15 mm                            │
└──────────────────────────────────────────────────────────────────────────┘
"""
...
def make_p200() -> cq.Workplane:
    """SLP-200: 左支撑条 — Left support bar

    Envelope: 40 x 260 x 15 mm (§10.3 visual table)
    Weight: ~421g (6061-T6, 2.70 g/cm³)

    Axis: Bar standing upright — 40(X) × 260(Y) × 15(Z)
    Doc:  §3.1 左支撑条, §10.3 视觉标识表 — 40×260×15 mm
          §3.1 spec says 50×100×8, but §10.3/§13 revised to 40×260×15
    """
    # ── Geometry: 40(X) × 260(Y) × 15(Z) rectangular bar ─────────────────────
    # In assembly: centered at X=−60, Z range [−8, 0] per §3.1 (original dims)
    # Using visual-table final dimensions 40×260×15
...
def _orientation_spec():
    ...
    Fill this in when implementing make_p200().
    ...
    return {
        "principal_axis": "z",
        "min_ratio": 1.0,
        "doc_ref": "§3.1 左支撑条 40×260×15",
    }


def draw_p200_sheet(output_dir: str = None) -> str:
    """Generate DXF three-view drawing for SLP-200.
    ...
    auto_annotate(solid, sheet, annotation_meta={
        ...
        "gdt": [],
        "surfaces": [{"material_type": "", "part": "\u5de6\u652f\u6491\u6761 SLP-200", "process": "6061-T6 \u94dd", "ra": "Ra3.2"}],
    })
...
if __name__ == "__main__":
    ...
    r = make_p200()
    p = os.path.join(out, "SLP-200.step")
