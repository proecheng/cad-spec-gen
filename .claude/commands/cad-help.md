...
3. **Photo3D / 照片级一键出图请求**（当用户说 photo3d、photo3d-run、photo3d-autopilot、photo3d-action、照片级、傻瓜式出图、pass/warning/blocked、accepted/preview/blocked、run_id、baseline、动作计划或让其他大模型继续时）：
   ...
   - 大模型优先运行/读取 `photo3d-run` 和 `PHOTO3D_RUN.json`；需要分步执行时只能依据 `ACTION_PLAN.json` 执行。低风险 CLI 恢复动作走 `photo3d-action` 预览/确认执行，底层命令必须经 `photo3d-recover --run-id <run_id> --artifact-index <path>` 绑定当前 run。如果动作需要用户输入，询问用户，不要虚构路径或模型。
