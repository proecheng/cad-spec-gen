...
| 5 | `gen_std_parts.py` | — | §5 BOM(外购件) + `parts_library.yaml` | `std_ee_NNN_NN.py` — 真实 STEP / 参数化库 / 简化兜底几何 |
...
**Parts Library 系统**（v2.8.0+，v2.21.2 几何质量闭环）：外购件不再硬编码到 `_gen_*` 简化几何，而是通过 `parts_resolver.PartsResolver` 路由到真实模型、参数化模型或末级兜底几何：
- **`StepPoolAdapter`**：从项目本地 `std_parts/` 目录加载真实 vendor STEP 文件（Maxon / LEMO / ATI 等），也可从共享缓存 `~/.cad-spec-gen/step_cache/` 读取/合成 vendor stand-in STEP
- **`SwToolboxAdapter`**：从本机 SolidWorks Toolbox 缓存/导出 GB/ISO/DIN 标准件 STEP；`inspect` / `probe` 模式缓存未命中时不触发 COM 导出
...
**§6.4 P7 包络回填**（v2.8.0+）：Phase 1 的 `cad_spec_gen.py` 在 P5/P6 后新增 P7 backfill 循环，对每个外购件调 `resolver.probe_dims()` 把库探测到的尺寸写入 §6.4，标签 `P7:STEP` / `P7:BW` / `P7:sw_toolbox` / `P7:PC`。优先级：
...
**Geometry quality report（v2.21.2+）**：`gen_std_parts.py` 在生成 `std_*.py` 的同一次 resolver 决策后写出 `cad/<subsystem>/.cad-spec-gen/geometry_report.json`。报告使用 `GeometryDecision` / `ResolveResult` 中的 `geometry_source`、`geometry_quality`、`validated`、`hash`、`path_kind`、`requires_model_review` 字段，按 A-E 质量分级展示：

| 质量 | 典型来源 | 含义 |
|---|---|---|
| A | 真实 STEP / SW Toolbox STEP | 最接近实物或标准库模型 |
| B | `bd_warehouse` / `partcad` | 参数化库模型，尺寸可信 |
| C | 半参数模板或未完全验证 STEP | 比包络占位更可辨识 |
| D | `jinja_primitive` | 简化占位，仅供粗略可视化 |
| E | missing | 无可用几何 |

`resolve_report.json` 也从同一决策日志生成；独立诊断场景只能使用 `mode="inspect"` 只读兜底，不允许为了报告再次触发 COM 导出、STEP 合成或缓存写入。
...
`gen_std_parts.py` 从 BOM 中提取所有 `外购` 零件，按名称/型号自动分类后优先解析真实/参数化模型；只有所有模型源都未命中时才生成简化 CadQuery 兜底几何：

模型源优先级由有效 `parts_library.yaml` 的 `mappings:` 决定（first-hit-wins）。推荐顺序是项目本地 STEP / 用户指定 STEP → SW Toolbox STEP → `bd_warehouse` / `partcad` → `jinja_primitive`。
