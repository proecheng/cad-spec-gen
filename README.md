...
**Latest: v2.10.0** — 结构性清理：data/ mirror 文件从 git tracking 移除，改由 `scripts/dev_sync.py` + pre-commit hook 同步（根治 v2.9.2 的 drift 问题）；移除 `_legacy_p2_regex_block` 和 `CAD_SPEC_WALKER_ENABLED` feature flag（section-header walker 已稳定三个版本）。488 全套测试零回归。See [`RELEASE_v2.10.0.md`](RELEASE_v2.10.0.md).
