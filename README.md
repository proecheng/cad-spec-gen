...
**Latest: v2.22.0** — `cad-skill-setup` now supports `--agent codex|both`, generating Codex-compatible `SKILL.md` files under `~/.agents/skills` from the canonical skill manifest and command docs. Claude Code slash-command installation remains the default. Version metadata still uses `pyproject.toml` as the single source of truth across `skill.json`, packaged `data/skill.json`, install markers, AGENTS, and README. See [`CHANGELOG.md`](CHANGELOG.md) and [`docs/PARTS_LIBRARY.md`](docs/PARTS_LIBRARY.md).
