...
**Latest: v2.23.3** — Patch release for end-effector model quality. The remaining D-grade primitive fallback parts now use C-grade semi-parametric Jinja templates, leaving the sample geometry report at `A=9, C=23, D=0` while preserving GATE-3.5 assembly validation at `0 WARNING` and F5 `33/33`. See [`CHANGELOG.md`](CHANGELOG.md) and [`docs/PARTS_LIBRARY.md`](docs/PARTS_LIBRARY.md).
