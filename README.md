...
**Latest: v2.22.1** — `cad-skill-setup` now supports `--agent codex|both`, generating Codex-compatible `SKILL.md` files under `~/.agents/skills` from the canonical skill manifest and command docs. Claude Code slash-command installation remains the default. This patch also makes STEP file selection gracefully return `None` when Tk/Tcl is unavailable on Windows CI or headless hosts. See [`CHANGELOG.md`](CHANGELOG.md) and [`docs/PARTS_LIBRARY.md`](docs/PARTS_LIBRARY.md).
