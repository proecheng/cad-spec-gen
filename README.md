...
**Latest: v2.23.0** — SolidWorks Toolbox model-library E2E validation is now available through `cad_pipeline.py sw-toolbox-e2e` and the manual `sw-smoke full=true` GitHub gate. This release verifies real Toolbox SLDPRT → STEP export and codegen STEP consumption on a self-hosted Windows runner, while tightening SW config matching, pending-decision propagation, and Codex skill tool packaging. See [`CHANGELOG.md`](CHANGELOG.md) and [`docs/PARTS_LIBRARY.md`](docs/PARTS_LIBRARY.md).
