...
**Latest: v2.23.1** — Patch release for model-library routing and sample geometry quality. `StepPoolAdapter` now supports safe `{normalize(name)}.step`-style templates without path escape, and sample scaffolds are cleaned of bare TODO placeholders while explicit connection-matrix axial gaps flow into generated CAD specs and assembly stacking. See [`CHANGELOG.md`](CHANGELOG.md) and [`docs/PARTS_LIBRARY.md`](docs/PARTS_LIBRARY.md).
