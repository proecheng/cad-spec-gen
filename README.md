...
**Latest: v2.9.2** — Test-only patch release: 渲染层回归覆盖 + 打包 mirror drift 安全网 + 历史 mirror 同步。77 个新测试（478 total pass）覆盖 `_resolve_camera_coords` 球坐标数学 / `view_sort_key` / `MATERIAL_PRESETS` PBR 结构契约 / `_get_bounding_sphere` v2.9.0 AABB fix 回归 / Blender 真实环境 import smoke；新增 `tests/test_data_dir_sync.py` 参数化 47 个 `(root, data/)` 同步对作为 drift 止血；新增 `blender` pytest marker 与 `slow` 职责分离。历史 drift 的 8 个 mirror + 3 个未追踪文件一次性追平入 HEAD。See [`RELEASE_v2.9.2.md`](RELEASE_v2.9.2.md).
