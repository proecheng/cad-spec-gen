...
**Latest: v2.23.2** — Patch release for assembly coherence. CAD_SPEC render exclusions now flow consistently into generated assemblies and GATE-3.5 validation, so excluded cables/connectors no longer appear as zero-offset solids; the end-effector sample stack now validates with 0 warnings and F5 expected/actual part counts aligned. See [`CHANGELOG.md`](CHANGELOG.md) and [`docs/PARTS_LIBRARY.md`](docs/PARTS_LIBRARY.md).
